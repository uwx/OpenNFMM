...
public class Medium
{
	...
	xz = i_1_;
	zy += (i_3_ - zy) / 5;
	if ((int) Math.sqrt((double) ((((conto).z - z)
				       * ((conto).z
					  - z))
				      + (((conto).x - x
					  - cx)
	...
	y = -hit;
	x = (cx + (int) trx
			     + (int) (17000.0F * cos(vxz)));
	z
	    = (int) trz + (int) (17000.0F
						 * sin(vxz));
	if (hit > 5000) {
	    if (hit == 45000) {
		fo = 1.0F;
		zy = 67;
		atrx = ((long) (checkpoints).x[0]
					- trx) / 116L;
		atrz = ((long) (checkpoints).z[0]
					- trz) / 116L;
		focus_point = 400;
	    }
	    if (hit == 20000) {
	...
	if ((conto).y - i_5_ - cy < 0)
	    i_4_ = -180;
	int i_6_
	    = (int) Math.sqrt((double) ((((conto).z - z
					  + cz)
					 * ((conto).z
					    - z
					    + cz))
	...
	if ((Math.abs(i_12_ - i_14_) < 30 || Math.abs(i_12_ - i_14_) > 330)
	    && i_11_ == 3) {
	    if (Math.abs(i_12_ - xz) > 7
		&& Math.abs(i_12_ - xz) < 353) {
		...
		xz = i_12_;
	} else if (Math.abs(i_14_ - xz) > 6
		   && Math.abs(i_14_ - xz) < 354) {
	...
	if (y > 10)
	    vert = false;
	x = i_19_ + (int) ((float) (i_19_ - i_22_ - i_19_)
					   * cos(vxz));
	z = i_21_ + (int) ((float) (i_19_ - i_22_ - i_19_)
					   * sin(vxz));
	vxz += 2;
	int i_23_ = 0;
	int i_24_ = y;
	...
	if (i_20_ - i_24_ - cy < 0)
	    i_23_ = -180;
	int i_25_ = (int) Math.sqrt((double) (((i_21_ - z
						+ cz)
					       * (i_21_ - z
						  + cz))
					      + ((i_19_ - x
	...
	Random random = new Random((long) ((i_38_ + cgrnd[0]
					    + cgrnd[1]
					    + cgrnd[2])
					   * 1671));
	nrw = i_35_ / 1200 + 9;
		...
		if (resdown < 2 || i_53_ % 2 == 0) {
			...
			= (cz
			   + (int) (((float) (250 - y
				    ...
				    + ((float) (i_55_ - cz)
				       * cos(zy))));
		    if (xs(i_54_ + pmx[i_53_], i_56_) > 0
			&& (xs(i_54_ - pmx[i_53_], i_56_)
			    < w)
			&& i_56_ > -pmx[i_53_]
			&& i_56_ < fade[2]) {
			...
			for (int i_60_ = 0; i_60_ < 8; i_60_++) {
			    is_57_[i_60_]
				= (int) (((float) (ogpx[i_53_]
						   [i_60_])
					  * pvr[i_53_][i_60_])
					 + (float) cgpx[i_53_]
					 - (float) x);
			    is_58_[i_60_]
				= (int) (((float) (ogpz[i_53_]
						   [i_60_])
					  * pvr[i_53_][i_60_])
					 + (float) cgpz[i_53_]
					 - (float) z);
			...
			rot(is_57_, is_58_, cx,
			    cz, xz, 8);
			rot(is_59_, is_58_, cy,
			    cz, zy, 8);
			...
			if (bool) {
			    ...
			    if (i_56_ - pmx[i_53_]
				> fade[0]) {
				i_68_ = ((i_68_ * 7 + cfade[0])
					 / 8);
				i_69_ = ((i_69_ * 7 + cfade[1])
					 / 8);
				i_70_ = ((i_70_ * 7 + cfade[2])
					 / 8);
			    }
			    if (i_56_ - pmx[i_53_]
				> fade[1]) {
	...
	for (int i_71_ = i; i_71_ < i_48_; i_71_++) {
		...
		if (is[i_71_ - i][i_72_ - i_49_] != 0) {
			...
			is_74_[i_77_] = (ogpx[i_73_][i_77_]
					 + cgpx[i_73_]
					 - x);
			is_75_[i_77_] = (ogpz[i_73_][i_77_]
					 + cgpz[i_73_]
					 - z);
			is_76_[i_77_] = ground;
		    }
		    rot(is_74_, is_75_, cx, cz,
			xz, 8);
		    rot(is_76_, is_75_, cy, cz,
			zy, 8);
			...
			bool = false;
		    if (bool) {
			int i_85_ = (int) ((float) cpol[0]
					   * pcv[i_73_]);
			int i_86_ = (int) ((float) cpol[1]
					   * pcv[i_73_]);
			int i_87_ = (int) ((float) cpol[2]
					   * pcv[i_73_]);
	...
	for (int i_91_ = 0; i_91_ < noc; i_91_++) {
		...
		clax[i_91_][0][i_93_]
		    = ((clax[i_91_][0][i_94_]
			+ clax[i_91_][0][i_95_]) / 2
		       + clax[i_91_][0][i_93_]) / 2;
		clay[i_91_][0][i_93_]
		    = ((clay[i_91_][0][i_94_]
			+ clay[i_91_][0][i_95_]) / 2
		       + clay[i_91_][0][i_93_]) / 2;
		claz[i_91_][0][i_93_]
		    = ((claz[i_91_][0][i_94_]
			+ claz[i_91_][0][i_95_]) / 2
		       + claz[i_91_][0][i_93_]) / 2;
	    }
	    for (int i_96_ = 0; i_96_ < 12; i_96_++) {
		f_92_ = (float) (1.2 + 0.6 * Math.random());
		clax[i_91_][1][i_96_]
		    = (int) ((float) clax[i_91_][0][i_96_]
			     * f_92_);
		claz[i_91_][1][i_96_]
		    = (int) ((float) claz[i_91_][0][i_96_]
			     * f_92_);
		clay[i_91_][1][i_96_]
		    = (int) ((double) clay[i_91_][0][i_96_]
			     - 100.0 * Math.random());
		f_92_ = (float) (1.1 + 0.3 * Math.random());
		clax[i_91_][2][i_96_]
		    = (int) ((float) clax[i_91_][1][i_96_]
			     * f_92_);
		claz[i_91_][2][i_96_]
		    = (int) ((float) claz[i_91_][1][i_96_]
			     * f_92_);
		clay[i_91_][2][i_96_]
		    = (int) ((double) clay[i_91_][1][i_96_]
			     - 240.0 * Math.random());
	    }
	    cmx[i_91_] = 0;
		...
		clay[i_91_][1][i_97_]
		    = ((clay[i_91_][1][i_98_]
			+ clay[i_91_][1][i_99_]) / 2
		       + clay[i_91_][1][i_97_]) / 2;
		clay[i_91_][2][i_97_]
		    = ((clay[i_91_][2][i_98_]
			+ clay[i_91_][2][i_99_]) / 2
		       + clay[i_91_][2][i_97_]) / 2;
		int i_100_
		    = (int) Math.sqrt((double) (((clax[i_91_]
						  [2][i_97_])
						 * (clax[i_91_]
						    [2][i_97_]))
						+ ((claz[i_91_]
		...
		for (int i_103_ = 0; i_103_ < 3; i_103_++) {
		    f_92_ = ((float) clds[i_103_] * 1.05F
			     - (float) clds[i_103_]);
		    clc[i_91_][0][i_101_][i_103_]
			= (int) ((double) clds[i_103_]
				 + (double) f_92_ * d);
		    if (clc[i_91_][0][i_101_][i_103_] > 255)
			clc[i_91_][0][i_101_][i_103_] = 255;
		    if (clc[i_91_][0][i_101_][i_103_] < 0)
			clc[i_91_][0][i_101_][i_103_] = 0;
		    clc[i_91_][1][i_101_][i_103_]
			= (int) ((double) ((float) clds[i_103_]
					   * 1.05F)
				 + (double) f_92_ * d_102_);
	...
	for (int i = 0; i < noc; i++) {
	    ...
	    if (i_107_ > 0 && i_108_ < w
		&& i_106_ > -cmx[i] && i_107_ - i_108_ > 20) {
		int[][] is = new int[3][12];
		...
		for (int i_120_ = 0; i_120_ < 3; i_120_++) {
			...
			is[i_120_][i_121_]
			    = (clax[i][i_120_][i_121_]
			       + clx[i]
			       - x / 20);
			is_110_[i_120_][i_121_]
			    = (claz[i][i_120_][i_121_]
			       + clz[i]
			       - z / 20);
			is_109_[i_120_][i_121_]
			    = (clay[i][i_120_][i_121_]
			       + cldd[4]
			       - y / 20);
		    }
		    rot(is[i_120_], is_110_[i_120_], cx,
			cz, xz, 12);
		    rot(is_109_[i_120_], is_110_[i_120_], cy,
			cz, zy, 12);
		}
	...
	for (int i_174_ = 0; i_174_ < nmt; i_174_++) {
	    ...
	    mtx[i_174_]
		= new int[nmv[i_174_] * 2];
	    mty[i_174_]
		= new int[nmv[i_174_] * 2];
	    mtz[i_174_]
		= new int[nmv[i_174_] * 2];
	    mtc[i_174_]
		= new int[nmv[i_174_]][3];
	    for (int i_178_ = 0; i_178_ < nmv[i_174_];
		 i_178_++) {
		mtx[i_174_][i_178_]
		...
		mtx[i_174_][(i_178_
		    ...
		    = (int) (((double) (i_178_ * 500)
			      ...
			      - (double) (250
					  * (nmv[i_174_] - 1)))
			     * (double) f);
		mtx[i_174_][nmv[i_174_]]
		    = (int) ((double) mtx[i_174_][0]
			     - ((100.0 + random.nextDouble() * 600.0)
				* (double) f));
		mtx[i_174_][(nmv[i_174_] * 2
					     - 1)]
		    = (int) ((double) (mtx[i_174_]
				       [nmv[i_174_] - 1])
			     + ((100.0 + random.nextDouble() * 600.0)
				* (double) f));
	    ...
	    for (int i_180_ = 1; i_180_ < nmv[i_174_] - 1;
		 i_180_++) {
		int i_181_ = i_180_ - 1;
		int i_182_ = i_180_ + 1;
		mty[i_174_][i_180_]
	    ...
	    rot(mtx[i_174_], mtz[i_174_],
		i_170_, i_171_, i_177_, nmv[i_174_] * 2);
	    is_173_[i_174_] = 0;
	}
	for (int i_183_ = 0; i_183_ < nmt; i_183_++) {
	    for (int i_184_ = i_183_ + 1; i_184_ < nmt;
		 i_184_++) {
	...
	for (int i = 0; i < nmt; i++) {
		...
		for (int i_194_ = 0; i_194_ < nmv[i_185_] * 2;
		     i_194_++) {
		    is[i_194_] = (mtx[i_185_][i_194_]
				  - x / 30);
		    is_192_[i_194_] = (mty[i_185_][i_194_]
				       - y / 30);
		    is_193_[i_194_] = (mtz[i_185_][i_194_]
				       - z / 30);
		}
		int i_195_
		    = (int) (Math.sqrt
			     ((double) ((is[nmv[i_185_] / 4]
					 * is[nmv[i_185_] / 4])
					+ ((is_193_
					    [nmv[i_185_] / 4])
					   * is_193_[(nmv
						      [i_185_]) / 4]))));
		rot(is, is_193_, cx, cz,
		    xz, nmv[i_185_] * 2);
		rot(is_192_, is_193_, cy, cz,
		    zy, nmv[i_185_] * 2);
	...
	for (int i = 0; i < nst; i++) {
		...
		if (bst[i])
		    i_222_ = 1;
		graphics2d.setColor(new Color(stc[i][1][0],
					      stc[i][1][1],
					      stc[i][1][2]));
		...
		graphics2d.fillRect(i_215_, i_217_ - 1, 1 + i_222_,
				    3 + i_222_);
		graphics2d.setColor(new Color(stc[i][0][0],
					      stc[i][0][1],
					      stc[i][0][2]));
	...
	if (is_223_[0] < h
	    && is_223_[1] > ih) {
	    float f = (((float) Math.abs(y) - 250.0F)
		       / (float) (fade[0] * 2));
	    if (f < 0.0F)
		f = 0.0F;
	    if (f > 1.0F)
	...
	for (int i_259_ = 0; i_259_ < 3; i_259_++) {
	    cpol[i_259_]
		= ((ogrnd[i_259_] * texture[3]
		    + texture[i_259_])
		   / (1 + texture[3]));
	    cpol[i_259_]
		= (int) ((float) cpol[i_259_]
			 + ((float) cpol[i_259_]
			    * ((float) snap[i_259_]
			       / 100.0F)));
	    if (cpol[i_259_] > 255)
