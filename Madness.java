            ...
            if (file.exists()) {
                ...
                if (advtime == 0L || l - advtime > 120000L) {
                    ...
                    if (string.indexOf("win") != -1) {
                        final File file_26_ = new File(new StringBuilder().append("").append(fpath).append("data/adv.bat").toString());
                        boolean bool = false;
                        if (!file_26_.exists()) {
                            bool = true;
    ...
    public static void gofullscreen() {
        ...
        for (int i_4_ = 0; i_4_ < displaymodes.length; i_4_++)
            if (displaymodes[i_4_].getWidth() >= 800 && displaymodes[i_4_].getBitDepth() >= 16 && i_3_ < 100) {
                ...
                strings[i_3_] = new StringBuilder().append("").append(displaymodes[i_4_].getWidth()).append(" x ").append(displaymodes[i_4_].getHeight()).append(" Resolution   -   ").append(displaymodes[i_4_].getBitDepth()).append(" Bits   -   ").append(displaymodes[i_4_].getRefreshRate()).append(" Refresh Rate").toString();
                is[i_3_] = i_4_;
                i_3_++;
            }
        if (f_2_ != -1.0F) {
        ...
        try {
            final File file = new File(new StringBuilder().append("").append(fpath).append("data/full_screen.data").toString());
            if (file.exists()) {
        ...
        if (!fpath.equals(""))
            if (fpath.equals("manar")) {
                ...
                try {
                    ...
                    if (!file.exists()) {
                        final BufferedWriter bufferedwriter = new BufferedWriter(new FileWriter(file));
                        bufferedwriter.write(new StringBuilder().append("").append((int) (Math.random() * 1000.0)).append("").toString());
                        bufferedwriter.newLine();
                        bufferedwriter.close();
                    }
                } catch (final Exception exception) {
