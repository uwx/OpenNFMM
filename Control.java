...
public class Control
{
	...
	if ((checkpoints).stage == 22) {
	    if (i != 13) {
		this.hold
		    = (int) (20.0F + 10.0F * this.m.random());
		this.revstart
		    = (int) (10.0F + 10.0F * this.m.random());
	    } else
		this.hold = 5;
	    this.statusque = 0;
	}
	...
	if (!(mad).dest) {
		...
		if (this.stcnt > this.statusque) {
			...
			this.trickprf = -1.0F;
		    if ((checkpoints).stage == 6
			&& (double) this.trickprf > 0.5)
			this.trickprf = 0.5F;
		    if ((checkpoints).stage == 3
			&& (mad).cn == 6
			&& (double) this.trickprf > 0.7)
			this.trickprf = 0.7F;
		    if ((checkpoints).stage == 13
			&& (mad).cn == 9
			&& (double) this.trickprf > 0.7)
			this.trickprf = 0.7F;
		    if ((checkpoints).stage == 16
			&& (double) this.trickprf > 0.3)
			this.trickprf = 0.3F;
		    if ((checkpoints).stage == 18
			&& (double) this.trickprf > 0.2)
			this.trickprf = 0.2F;
		    if ((checkpoints).stage == 19) {
		...
		if (this.wall != -1) {
			...
			this.right = true;
		    if ((trackers).dam[this.wall]
			!= 0) {
			int i_45_ = 1;
			if ((trackers).skd[this.wall]
			    == 1)
			    i_45_ = 3;
			this.hold += i_45_;
