...
public class Trails {
    ...
    public static class ExampleTrail implements Trail {
        ...
        @Override
        public void draw(Graphics2D rd, int xz, int xy, int zy, int tx, int ty, int tz, int nx, int ny, int nz) {
            int sin = (int) (MathUtil.sin(xz) * 50f);
            int cos = (int) (MathUtil.cos(xz) * 50f);
            
            final int aX[] = { tx - sin, tx + sin };
            final int aY[] = { ty,       ty       };
            final int aZ[] = { tz - cos, tz + cos };
