        ...
        try {
            ...
            while ((string187 = datainputstream.readLine()) != null) {
                ...
                if (line.startsWith("nlaps")) {
                    CheckPoints.nlaps = getint("nlaps", line, 0);
...
//                }
                if (line.startsWith("chk")) {
                    ...
                    co[nob].partID = i204;
                    CheckPoints.x[CheckPoints.n] = getint("chk", line, 1);
                    CheckPoints.z[CheckPoints.n] = getint("chk", line, 2);
                    CheckPoints.y[CheckPoints.n] = getint("chk", line, 3);
                ...
                if (line.startsWith("fix")) {
                    ...
                    co[nob].partID = i208;
                    CheckPoints.fx[CheckPoints.fn] = getint("fix", line, 1);
                    CheckPoints.fz[CheckPoints.fn] = getint("fix", line, 2);
                    CheckPoints.fy[CheckPoints.fn] = getint("fix", line, 3);
                    //co[nob].elec = true;
            ...
            Medium.newpolys(i182, i181 - i182, i184, i183 - i184, Trackers, nob);
    ...
    SRCStageMaker() {
        ...
        asin_m_zy = Medium.sin(Medium.zy);
        acos_m_zy = Medium.cos(Medium.zy);
    ...
    private static void gameTick() {
        ...
        if (tab == 1) {
                ...
                for (int i8 = 0; i8 < i; i8++)
                    if (is3[i8] == i7) {
                        ...
                        if (is[i8] == chi && !co[is[i8]].errd) {
                            ...
                            rd.fillOval(Utility.xs(i9, i12, Medium) - i13 / 2, Utility.ys(i11, i12, Medium) - i13 / 2, i13, i13);
                            graphics2d.setComposite(AlphaComposite.getInstance(3, 1.0F));
                            rd.setColor(Color.black);
                            rd.setFont(FontConstants.arial_12);
                            ftm = rd.getFontMetrics();
                            rd.drawString("NO# " + (arrcnt + 1), Utility.xs(i9, i12, Medium) - ftm.stringWidth("NO# " + (arrcnt + 1)) / 2, Utility.ys(i11, i12, Medium) - i13 / 2);
                        }
                        ...
                        if (arrng && (co[is[i8]].partID == ONROAD_CHECKPOINT_SET_ID || Utility.arrayContains(CHECKPOINT_IDS, co[is[i8]].partID) || co[is[i8]].partID == OFFROAD_CHECKPOINT_SET_ID || co[is[i8]].partID == FLYING_CHECKPOINT_SET_ID) && co[is[i8]].errd) {
                            ...
                            rd.fillOval(Utility.xs(i14, i17, Medium) - i18 / 2, Utility.ys(i16, i17, Medium) - i18 / 2, i18, i18);
                            graphics2d.setComposite(AlphaComposite.getInstance(3, 1.0F));
                            rd.setColor(Color.black);
                            rd.setFont(FontConstants.arial_12);
                            ftm = rd.getFontMetrics();
                            rd.drawString("NO# " + co[is[i8]].wh, Utility.xs(i14, i17, Medium) - ftm.stringWidth("NO# " + co[is[i8]].wh) / 2, Utility.ys(i16, i17, Medium) - i18 / 2);
                        }
                        co[is[i8]].d(rd);
                        if (Medium.trk == 3) {
                            Medium.trk = 2;
                    ...
                    if (arrng) {
                        ...
                        for (int i48 = 0; i48 < nob; i48++)
                            if ((co[i48].partID == ONROAD_CHECKPOINT_SET_ID || Utility.arrayContains(CHECKPOINT_IDS, co[i48].partID) || co[i48].partID == OFFROAD_CHECKPOINT_SET_ID || co[i48].partID == FLYING_CHECKPOINT_SET_ID) && !co[i48].errd) {
                                ...
                                if (xm > Utility.xs(i49 - co[i48].maxR, i52, Medium) && xm < Utility.xs(i49 + co[i48].maxR, i52, Medium) && ym > Utility.ys(i51 - co[i48].maxR, i52, Medium) && ym < Utility.ys(i51 + co[i48].maxR, i52, Medium) && py(xm, Utility.xs(i49, i52, Medium), ym, Utility.ys(i51, i52, Medium)) <= i47) {
                                    chi = i48;
                                    i47 = py(xm, Utility.xs(i49, i52, Medium), ym, Utility.ys(i51, i52, Medium));
                                }
                            }
                        if (chi != -1) {
                            if (!setcur) {
            ...
            if (epart && esp != -1)
                if (co[esp].dist != 0) {
                    ...
                    final int i57 = Utility.xs(i53, i56, Medium);
                    final int i58 = Utility.ys(i55, i56, Medium);
                    rd.setColor(Color.white);
        ...
        if (tab == 2) {
            ...
            if (up) {
                if (kSettings) {
                    uAccFloat = 1.0F;
                    vz += 50.0F * Medium.cos(Medium.xz);
                    vx += 50.0F * Medium.sin(Medium.xz);
                } else {
                    vz += 250.0F * Medium.cos(Medium.xz);
                    vx += 250.0F * Medium.sin(Medium.xz);
                }
            }
            if (down) {
                if (kSettings) {
            ...
            if (kSettings) {
                if (uAccFloat > 0.0F) {
                    vz += 200.0F * Medium.cos(Medium.xz) * speedShift * uAccFloat;
                    vx += 200.0F * Medium.sin(Medium.xz) * speedShift * uAccFloat;
                    uAccFloat -= 0.1F;
                }
                if (dAccFloat > 0.0F) {
                    vz -= 200.0F * Medium.cos(Medium.xz) * speedShift * dAccFloat;
                    vx -= 200.0F * Medium.sin(Medium.xz) * speedShift * dAccFloat;
                    dAccFloat -= 0.1F;
                }
                if (lAccFloat > 0.0F) {
    ...
    private static void drawSelection() {
        ...
        x2d[1] = Utility.xs(aX[1], aZ[1], Medium);
        y2d[1] = Utility.mediumYs(aY[1], aZ[1], Medium);
    ...
    private static void getHighlightedPiece() {
        ...
        for (int i42 = 0; i42 < nob; i42++) {
            ...
            if (xm > Utility.xs(i43 - co[i42].maxR, i46, Medium) && xm < Utility.xs(i43 + co[i42].maxR, i46, Medium) && ym > Utility.ys(i45 - co[i42].maxR, i46, Medium) && ym < Utility.ys(i45 + co[i42].maxR, i46, Medium) && co[i42].partID != 37 && co[i42].partID != 38)
                if (hi == -1) {
                    hi = i42;
                    i41 = py(xm, Utility.xs(i43, i46, Medium), ym, Utility.ys(i45, i46, Medium));
                } else if (py(xm, Utility.xs(i43, i46, Medium), ym, Utility.ys(i45, i46, Medium)) <= i41) {
                    hi = i42;
                    i41 = py(xm, Utility.xs(i43, i46, Medium), ym, Utility.ys(i45, i46, Medium));
                }
        }
    ...
    private static void drawpartids() {
        if (toggle_drawpartids) {
            ...
            for (int b = 0; b < nob; b++) {
                ...
                //bco[selectedPart].z = (290 - ym) * (Math.abs(sy) / Medium.focusPoint) + sz;
                //
                int x = co[b].x - Medium.x - Medium.cx;// / 30;
                int y = co[b].y - Medium.y - Medium.cy;// / 30;
                int z = co[b].z - Medium.z - Medium.cz;// / 30;
                int[] a = rotSingle(x, z, Medium.cx, Medium.cz, Medium.xz, asin_m_xz, acos_m_xz);
                x = a[0];
                z = a[1];
                a = rotSingle(y, z, Medium.cy, Medium.cz, Medium.zy, asin_m_zy, acos_m_zy);
                y = a[0];
                z = a[1];
                a = rotSingle(x, y, Medium.cx, Medium.cy, __m_xy, asin_m_xy, acos_m_xy);
                x = a[0];
                y = a[1];
                final int x2d = Utility.xs(x, z, Medium);
                final int y2d = Utility.ys(y, z, Medium); //mediumys maybe
    ...
    private static void drawNodePath() {
        ...
        for (int i = 0; i < CheckPoints.n - 1; i++) {
            /*if (playerPos[i].dist == 0) {
                playerPos[i].d(rd);
            }
            playerPos[i].d(rd);*/
