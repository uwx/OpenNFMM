...
/**
 * RadicalBASS variant using direct mapped memory (DirectByteBuffer), not tested
 *
 * @author Rafael
 */
public class RadicalBASS_direct implements RadicalMusic {
    ...
    private void run() {
        ...
        try (ZipInputStream z = new ZipInputStream(new FileInputStream(file))) {
            ...
            if (entrySize == -1) {
                ...
                song = ByteBuffer.allocateDirect(songSize);
                song.put(ubos.getDataBuffer()); // this MUST copy the array, since we're dealing with direct memory data...
                ...
                song = ByteBuffer.allocateDirect(entrySize);
                songSize = entrySize;
    ...
    private boolean playFile() {
        ...
        final HSTREAM stream = BASS_StreamCreateFile(true, song, 0, songSize, 0);
        HMUSIC music = null;

        if (stream == null) { // it's a MOD/MO3 file
            //error("Can't play file (stream)");

            music = BASS_MusicLoad(true, BufferUtils.asPointer(song), 0, songSize, BASS_MUSIC_RAMPS | BASS_MUSIC_POSRESET | BASS_MUSIC_PRESCAN, 0);
     ...
     */
    public RadicalBASS_direct(final File songFile) {
        file = songFile;
        //run();
    }
