...
public class GameSparker extends Applet implements Runnable {
	...
	public void run() {
		...
		final ContO[] contos = new ContO[124];
		final CarDefine cardefine = new CarDefine(contos, medium, trackers, this);
		final xtGraphics var_xtGraphics = new xtGraphics(medium, cardefine, rd, this);
		...
		final ContO[] contos_0_ = new ContO[10000];
		final Mad[] mads = new Mad[8];
		for (int i = 0; i < 8; i++) {
				...
				if ((var_xtGraphics).fase == 0) {
					for (int i_33_ = 0; i_33_ < (var_xtGraphics).nplayers; i_33_++)
						if ((mads[i_33_]).newcar) {
							...
							contos_0_[i_33_] = new ContO(contos[(mads[i_33_]).cn], (contos_0_[i_33_]).x,
									(contos_0_[i_33_]).y, (contos_0_[i_33_]).z, 0);
					...
					if ((var_xtGraphics).starcnt < 38) {
						...
						if ((var_xtGraphics).starcnt == 38) {
							...
							(medium).adv = 900;
							(medium).vxz = 180;
							checkpoints.checkstat(mads, contos_0_, record, ((var_xtGraphics).nplayers),
									(var_xtGraphics).im, 0);
				...
				if ((var_xtGraphics).fase == 7001) {
					for (int i_51_ = 0; i_51_ < (var_xtGraphics).nplayers; i_51_++)
						if ((mads[i_51_]).newedcar == 0 && (mads[i_51_]).newcar) {
							...
							contos_0_[i_51_] = new ContO(contos[(mads[i_51_]).cn], (contos_0_[i_51_]).x,
									(contos_0_[i_51_]).y, (contos_0_[i_51_]).z, 0);
					...
					for (int i_62_ = 0; i_62_ < i_55_; i_62_++) {
						if ((is[is_58_[i_62_]] < (var_xtGraphics).nplayers)
								&& (is[is_58_[i_62_]] != (var_xtGraphics).im))
					...
					if ((var_xtGraphics).starcnt == 0) {
						if ((var_xtGraphics).multion == 1) {
							...
							for (int i_65_ = 0; i_65_ < (var_xtGraphics).nplayers; i_65_++)
								udpmistro.readinfo(mads[i_65_], contos_0_[i_65_], u[i_65_], i_65_,
										((checkpoints).dested));
					...
					if ((var_xtGraphics).starcnt < 38) {
						if ((var_xtGraphics).multion == 1) {
							udpmistro.setinfo((mads[(var_xtGraphics).im]), (contos_0_[(var_xtGraphics).im]), u[0],
									((checkpoints).pos[(var_xtGraphics).im]),
									((checkpoints).magperc[(var_xtGraphics).im]), (var_xtGraphics).holdit,
									(var_xtGraphics).im);
							if (view == 0) {
								medium.follow((contos_0_[(var_xtGraphics).im]), ((mads[(var_xtGraphics).im]).cxz),
										((u[0])).lookback);
								var_xtGraphics.stat(mads[(var_xtGraphics).im], contos_0_[(var_xtGraphics).im],
						...
						if ((var_xtGraphics).starcnt == 38) {
							...
							(medium).vxz = 180;
							checkpoints.checkstat(mads, contos_0_, record, ((var_xtGraphics).nplayers),
									(var_xtGraphics).im, ((var_xtGraphics).multion));
	...
	public void loadstage(final ContO[] contos, final ContO[] contos_108_, final Medium medium, final Trackers trackers,
					...
					if (bool) {
						...
						if (string.indexOf(")p") != -1) {
							...
							if (string.indexOf("out") != -1)
								System.out.println(
										new StringBuilder().append("out: ").append((checkpoints).n).toString());
							(checkpoints).n++;
							notb = nob + 1;
						}
						nob++;
		...
		if ((checkpoints).stage != -3) {
			...
			if ((checkpoints).stage > 0) {
				...
				(var_xtGraphics).asay = new StringBuilder().append("Custom Stage:  ").append((checkpoints).name)
						.append(" ").toString();
			record.reset(contos);
	...
	@Override
	public boolean mouseUp(final Event event, final int i, final int i_203_) {
		if (!exwist) {
			if (mouses == 11) {
				xm = (int) ((i - apx) / apmult);
	...
	@Override
	public boolean mouseDown(final Event event, final int i, final int i_204_) {
		requestFocus();
		if (!exwist) {
			if (mouses == 0) {
				xm = (int) ((i - apx) / apmult);
	...
	@Override
	public boolean mouseMove(final Event event, final int i, final int i_205_) {
		if (!exwist && !lostfcs) {
			xm = (int) ((i - apx) / apmult);
	...
	@Override
	public boolean mouseDrag(final Event event, final int i, final int i_206_) {
		if (!exwist && !lostfcs) {
			xm = (int) ((i - apx) / apmult);
