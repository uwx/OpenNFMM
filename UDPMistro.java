    ...
    public void run() {
        ...
        while (runon == 1) {
            ...
            if (delay > rate && i <= delay / rate && i >= 1 && i < 20) {
                Date date = new Date();
                long l = date.getTime() - (udpc[i - 1]).sendat;
            ...
            if (diled == 10) {
                for (int i_18_ = 0; i_18_ < 20; i_18_++) {
                    if ((udpc[i_18_]).started) {
                        Date date = new Date();
                        long l = date.getTime() - (udpc[i_18_]).sendat;
                        if ((double) l > (double) delay * 1.5 && l > (long) rate)
                            udpc[i_18_].stomp();
                    }
                }
            }
        ...
        while (runon == 4) {
            for (int i_21_ = 0; i_21_ < nplayers; i_21_++) {
                ...
                if (info[i_21_][0].length() > 16) {
                    String string = new StringBuilder().append("").append(info[i_21_][0].charAt(15)).toString();
                    if (!string.equals("0"))
                        bool = true;
                }
            ...
            if (i_20_ == 10) {
                String string = new StringBuilder().append("3|").append(pgame).append("|alive|").toString();
                String string_22_ = "";
                if (i_19_ == 0) {
                    try {
                        dout.println(string);
