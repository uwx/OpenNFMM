...
public class xtGraphics extends Panel implements Runnable {
		...
		if (i == 4) {
			...
			for (int i_37_ = 0; i_37_ < 4; i_37_++) {
				...
				rd.fillOval((int) (65 + ovx[i_37_] - (ovw[i_37_] * 1.5 / 2.0)),
						...
						(int) (ovh[i_37_] * 1.5));
				rd.setColor(new Color(255, 176, 67));
				rd.fillOval((65 + ovx[i_37_] - ovh[i_37_] / 2), (25 + ovy[i_37_] - ovh[i_37_] / 2), ovw[i_37_],
						ovh[i_37_]);
		...
		if (testdrive != 3 && testdrive != 4) {
			...
			if (gmode == 0) {
						...
						if ((checkpoints).stage == -2 && ((cd).staction == 0)) {
							...
							if ((checkpoints).top20 >= 3)
								rd.drawString(new StringBuilder().append("Added by :  ")
										.append((cd).top20adds[((checkpoints).nto - 1)]).append(" Players").toString(),
										70, 135);
						}
					} else if (removeds != 1) {
						rd.setFont(new Font("Arial", 1, 13));
				...
				if (nfmtab == 3 || nfmtab == 4) {
					...
					if (!string.equals("") && !string.equals((checkpoints).name)
							&& (app).mstgs.getSelectedIndex() != 0) {
						(checkpoints).stage = -2;
				...
				if (aflk) {
					drawcs(100, new StringBuilder().append("| ").append((checkpoints).name).append(" |").toString(),
							240, 240, 240, 3);
					aflk = false;
				} else {
					drawcs(100, new StringBuilder().append("| ").append((checkpoints).name).append(" |").toString(),
				...
				if ((checkpoints).stage > 0) {
					if ((control).right) {
						if (gmode == 0 || (gmode == 1 && ((checkpoints).stage != unlocked[0]))
								|| (gmode == 2 && ((checkpoints).stage != unlocked[1] + 10))
								|| (checkpoints).stage == 27) {
							if ((checkpoints).stage != 27) {
								hidos();
								(checkpoints).stage++;
	...
	public void multistat(final Control control, final CheckPoints checkpoints, final int i, final int i_53_,
						...
						if (bool_59_) {
							...
							if (!string.equals("")) {
								...
								if (!msgcheck(string) && (updatec[i_57_] > -12)) {
									if (cnames[i_57_][6].equals("Game Chat  ")
											|| (cnames[i_57_][6].equals(new StringBuilder().append("").append(clan)
													.append("'s Chat  ").toString())))
										cnames[i_57_][6] = "";
				...
				for (int i_75_ = 0; i_75_ < i_74_; i_75_++) {
					...
					for (int i_77_ = 0; i_77_ < nplayers; i_77_++)
						if ((checkpoints).pos[i_77_] == i_75_ && (checkpoints).dested[i_77_] == 0 && !bool_76_) {
							...
							rd.setColor(new Color(0, 0, 0));
							rd.drawString(plnames[i_77_], 730 - ((ftm.stringWidth(plnames[i_77_])) / 2),
									70 + 30 * i_75_);
		...
		if (i_54_ != -1) {
			...
			rd.setColor(new Color((int) (((m).cgrnd[0]) / 1.2F), (int) (((m).cgrnd[1]) / 1.2F),
					(int) (((m).cgrnd[2]) / 1.2F)));
		...
		if (flipo == 0) {
			...
			if (!remi && (cfase != 10 || ((cd).action != 0 && ((cd).action < 14)))) {
				if (cfase == 3 && (cd).lastload == 2) {
					...
					if (aflk) {
						drawcs(95 + i_115_, new StringBuilder().append(string).append((cd).names[sc[0]]).toString(),
								240, 240, 240, 3);
						aflk = false;
					} else {
						drawcs(95, new StringBuilder().append(string).append((cd).names[sc[0]]).toString(), 176, 176,
								176, 3);
						aflk = true;
					}
				}
			...
			if (i_113_ != 0) {
				...
				if (flatrstart == 6) {
					if (cfase == 10) {
						...
						if ((cd).action == 12) {
							...
							drawcs(195,
									...
									0, 0, 0, 3);
							if ((cd).nl > 0 && ((cd).nl <= 20))
								drawcs(235, new StringBuilder().append("Loading :  ")
										.append((cd).loadnames[(cd).nl - 1]).append("").toString(), 0, 0, 0, 3);
						}
						if ((cd).action == 11) {
							drawprom(145, 170);
					...
					f = (((cd).acelf[sc[0]][1]) * ((cd).acelf[sc[0]][0]) * ((cd).acelf[sc[0]][2]) * ((cd).grip[sc[0]])
							/ 7700.0F);
					if (f > 1.0F)
						f = 1.0F;
						...
						if (bool) {
							if (mouson == -1) {
								if (i > 96 && i < 152 && i_104_ > 248 && i_104_ < 258) {
									final float[] fs = new float[3];
									Color.RGBtoHSB((contos[(sc[0])]).fcol[0], (contos[(sc[0])]).fcol[1],
											(contos[(sc[0])]).fcol[2], fs);
									arnp[0] = fs[0];
									arnp[1] = fs[1];
									arnp[2] = 1.0F - fs[2];
								}
								if (i > 646 && i < 702 && i_104_ > 248 && i_104_ < 258) {
									final float[] fs = new float[3];
									Color.RGBtoHSB((contos[(sc[0])]).scol[0], (contos[(sc[0])]).scol[1],
											(contos[(sc[0])]).scol[2], fs);
									arnp[3] = fs[0];
									arnp[4] = fs[1];
									arnp[5] = 1.0F - fs[2];
								}
				...
				if (!remi && cfase != 10 && cfase != 11 && cfase != 100 && cfase != 101)
					...
					if (cfase == 101 && i_112_ == -1)
						if (((cd).publish[sc[0] - 16]) == 1 || ((cd).publish[sc[0] - 16]) == 2) {
							if (drawcarb(true, null, "Add to My Cars", 345, 385, i, i_104_, bool)
									&& ((cd).action == 0)) {
								(cd).ac = sc[0];
								if (logged) {
									(cd).action = 6;
				...
				if (flipo == 10) {
					...
					if (nextc == 1) {
						...
						if (multion != 0 && onjoin != -1 && ontyp > 0 && ontyp <= 5)
							for ( /**/ ; ((sc[0] < maxsl)
									&& Math.abs(((cd).cclass[sc[0]]) - (ontyp - 1)) > 1); sc[0]++) {
								/* empty */
							}
					}
					if (nextc == -1) {
		...
		if (multion < 3) {
			if (winner) {
				...
				drawcs(137, new StringBuilder().append("You Won!  At Stage").append(string).append("").toString(), 255,
						161, 85, 3);
				drawcs(154, new StringBuilder().append("").append((checkpoints).name).append("").toString(), 255, 115,
						0, 3);
				i_142_ = 154;
			} else {
				...
				if (multion != 0 && (forstart == 700 || discon == 240)) {
					...
					drawcs(167, new StringBuilder().append("You Lost!  At Stage").append(string).append("").toString(),
							255, 161, 85, 3);
					drawcs(184, new StringBuilder().append("").append((checkpoints).name).append("").toString(), 255,
							115, 0, 3);
					i_142_ = 184;
				}
			}
			rd.setColor(new Color(193, 106, 0));
			rd.drawString((new StringBuilder()).append("Your time: ").append(chrono.getTotalTime()).toString(),
					380 - ftm.stringWidth(
							(new StringBuilder()).append("Your time: ").append(chrono.getTotalTime()).toString()),
					200);
			...
			drawcs(184, new StringBuilder().append("").append((checkpoints).name).append("").toString(), 255, 115, 0,
					3);
			i_142_ = 184;
		}
				...
				if ((checkpoints).stage != 10 && multion < 2 && nplayers != 1 && (arrace != (control).arrace)) {
					...
					if (arrace) {
						...
						if (multion == 1) {
							...
							final xtGraphics var_xtGraphics_190_ = this;
					...
					if (!arrace) {
						...
						if (multion == 1) {
							final StringBuilder stringbuilder = new StringBuilder();
							final xtGraphics var_xtGraphics_191_ = this;
				...
				for (int i = 0; i < nplayers; i++)
					if ((dested[i] != (checkpoints).dested[i]) && i != im) {
						...
						if (fase != 7001) {
							if (dested[i] == 1) {
								wasay = true;
								say = new StringBuilder().append("").append((cd).names[sc[i]])
		...
		if (!bool) {
			...
			if (multion == 0 || alocked == -1) {
				...
				for (int i_229_ = 0; i_229_ < nplayers; i_229_++)
					if (i_229_ != im
							&& ((py(((checkpoints).opx[im]) / 100, (checkpoints).opx[i_229_] / 100,
									((checkpoints).opz[im]) / 100, (checkpoints).opz[i_229_] / 100) < i_227_)
									|| i_227_ == -1)
							&& (!bool_228_ || ((checkpoints).onscreen[i_229_] != 0))
							&& (checkpoints).dested[i_229_] == 0) {
						i_226_ = i_229_;
						i_227_ = py(((checkpoints).opx[im]) / 100, (checkpoints).opx[i_229_] / 100,
								((checkpoints).opz[im]) / 100, ((checkpoints).opz[i_229_] / 100));
			...
			if (((checkpoints).opx[i_226_] - (checkpoints).opx[im]) >= 0)
				i_230_ = 180;
			i_224_ = (int) (90 + i_230_
					+ ((Math.atan((double) ((checkpoints).opz[i_226_] - ((checkpoints).opz[im]))
							/ (double) (((checkpoints).opx[i_226_]) - ((checkpoints).opx[im]))))
			...
			for (int i_248_ = 0; i_248_ < nplayers; i_248_++)
				if (i_248_ != im && (checkpoints).dested[i_248_] == 0) {
					...
					if (alocked == i_248_) {
						...
						rd.setColor(new Color((i + ((m).csky[0])) / 2, (((m).csky[1]) + i_246_) / 2,
								(i_247_ + ((m).csky[2])) / 2));
	...
	public void playsounds(final Mad mad, final Control control, final int i) {
		if ((fase == 0 || fase == 7001) && starcnt < 35 && cntwis != 8 && !mutes) {
			...
			boolean bool_257_ = ((mad).skid == 1 && (control).handb
					|| Math.abs(
							(mad).scz[0] - (((mad).scz[1] + (mad).scz[0] + (mad).scz[2] + (mad).scz[3]) / 4.0F)) > 1.0F
					|| Math.abs((mad).scx[0]
							- (((mad).scx[1] + (mad).scx[0] + (mad).scx[2] + (mad).scx[3]) / 4.0F)) > 1.0F);
					...
					if (!bool_257_) {
						if ((mad).power != 98.0F) {
							...
							if ((Math.abs((mad).speed) > ((cd).swits[(mad).cn][1]))
								...
								final int i_261_ = (int) (3.0F
										* (Math.abs((mad).speed) - (((this).cd).swits[(mad).cn][1]))
					...
					if (smokey[i + i_407_ * 466] != smokey[0]) {
						...
						if (i + i_408_ + 100 < 670 && i + i_408_ + 100 > 0 && i_407_ + i_409_ + 110 < 400
							...
							int i_416_ = (int) (((color.getGreen() * (flang * f_413_))
									+ (color_411_.getGreen() * (1.0F - f_413_)))
									/ ((flang * f_413_) + (1.0F - f_413_)));
