...
public class Record
{
	...
	if (caught >= 300) {
		...
		for (int i_10_ = 0; i_10_ < 20; i_10_++) {
			...
			hsspark[i_9_][i_10_][i_11_]
			    = sspark[i_9_][i_10_][i_11_];
			hsx[i_9_][i_10_][i_11_]
			    = sx[i_9_][i_10_][i_11_];
			hsy[i_9_][i_10_][i_11_]
			    = sy[i_9_][i_10_][i_11_];
			hsz[i_9_][i_10_][i_11_]
			    = sz[i_9_][i_10_][i_11_];
			hsmag[i_9_][i_10_][i_11_]
			    = smag[i_9_][i_10_][i_11_];
			hscx[i_9_][i_10_][i_11_]
			    = scx[i_9_][i_10_][i_11_];
			hscz[i_9_][i_10_][i_11_]
			    = scz[i_9_][i_10_][i_11_];
		    }
		}
		...
		for (int i_14_ = 0; i_14_ < 4; i_14_++) {
			...
			hry[i_13_][i_14_][i_15_]
			    = ry[i_13_][i_14_][i_15_];
			hmagy[i_13_][i_14_][i_15_]
			    = magy[i_13_][i_14_][i_15_];
			hrx[i_13_][i_14_][i_15_]
			    = rx[i_13_][i_14_][i_15_];
			hmagx[i_13_][i_14_][i_15_]
			    = magx[i_13_][i_14_][i_15_];
			hrz[i_13_][i_14_][i_15_]
			    = rz[i_13_][i_14_][i_15_];
			hmagz[i_13_][i_14_][i_15_]
			    = magz[i_13_][i_14_][i_15_];
		    }
		}
	    }
	...
	if (cntf == 50) {
		...
		car[i_22_][i]
		    = new ContO(car[i_22_ + 1][i], 0, 0, 0, 0);
		squash[i_22_][i]
		    = squash[i_22_ + 1][i];
	    }
	...
	for (int i_25_ = 0; i_25_ < 20; i_25_++) {
		...
		sspark[i][i_25_][ns[i][i_25_]]
		    = 300;
		sx[i][i_25_][ns[i][i_25_]]
		    = (conto).sx[i_25_];
		sy[i][i_25_][ns[i][i_25_]]
		    = (conto).sy[i_25_];
		sz[i][i_25_][ns[i][i_25_]]
		    = (conto).sz[i_25_];
		smag[i][i_25_][ns[i][i_25_]]
		    = (conto).osmag[i_25_];
		scx[i][i_25_][ns[i][i_25_]]
		    = (conto).scx[i_25_];
		scz[i][i_25_][ns[i][i_25_]]
		    = (conto).scz[i_25_];
		ns[i][i_25_]++;
		if (ns[i][i_25_] == 30)
		    ns[i][i_25_] = 0;
	...
	for (int i_36_ = 0; i_36_ < 4; i_36_++) {
		...
		if (ry[i][i_36_][i_37_] == i_30_)
		    regy(i_36_, (float) magy[i][i_36_][i_37_],
			 mtouch[i][i_37_], conto, mad);
		if (rx[i][i_36_][i_37_] == i_30_)
		    regx(i_36_, (float) magx[i][i_36_][i_37_],
			 conto, mad);
		if (rz[i][i_36_][i_37_] == i_30_)
		    regz(i_36_, (float) magz[i][i_36_][i_37_],
			 conto, mad);
	    }
	}
    }
	...
	for (int i_45_ = 0; i_45_ < 4; i_45_++) {
		...
		if (hry[i][i_45_][i_46_] == i_38_
		    && lastfr != i_38_)
		    regy(i_45_, (float) hmagy[i][i_45_][i_46_],
			 hmtouch[i][i_46_], conto, mad);
		if (hrx[i][i_45_][i_46_] == i_38_) {
		    if (lastfr != i_38_)
			regx(i_45_,
			     (float) hmagx[i][i_45_][i_46_],
			     conto, mad);
		    else
			chipx(i_45_,
			      (float) hmagx[i][i_45_][i_46_],
