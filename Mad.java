...
public class Mad
{
	...
	this.forca
	    = (((float) Math.sqrt((double) (((conto).keyz[0]
					     * (conto).keyz[0])
					    + ((conto).keyx[0]
					       * (conto).keyx[0])))
		+ (float) Math.sqrt((double) (((conto).keyz[1]
					       * (conto).keyz[1])
					      + ((conto).keyx[1]
						 * (conto).keyx[1])))
		+ (float) Math.sqrt((double) (((conto).keyz[2]
					       * (conto).keyz[2])
					      + ((conto).keyx[2]
						 * (conto).keyx[2])))
		+ (float) Math.sqrt((double) (((conto).keyz[3]
					       * (conto).keyz[3])
					      + ((conto).keyx[3]
						 * (conto).keyx[3]))))
	       / 10000.0F
	       * (float) ((double) ((this.cd).bounce
				    [this.cn])
			  - 0.3));
	this.mtouch = false;
	this.wtouch = false;
	...
	if ((control).handb) {
	    ...
	    this.pushed = false;
	if (this.loop == 1) {
	    float f_13_ = ((this.scy[0] + this.scy[1]
			    + this.scy[2] + this.scy[3])
			   / 4.0F);
	...
	if (!this.dest) {
		...
		if ((control).up) {
			 ...
			 * this.m.sin((conto).xz)
			 * (float) i_4_);
		    f_11_ = ((float) ((this.cd).airc
				      [this.cn])
			     * this.m.cos((conto).xz)
			     * (float) i_4_);
		} else if (this.ucomp != 0.0F
			   && this.ucomp > -2.0F)
		    this.ucomp
			-= 0.5 * (double) ((this.cd).airs
					   [this.cn]);
		if ((control).down) {
		    if (this.dcomp == 0.0F) {
			this.dcomp
			...
			this.dcomp
			    += 0.5 * (double) ((this.cd)
					       .airs[this.cn]);
		    f_12_ = (float) -((this.cd).airc
				      [this.cn]);
		} else if (this.dcomp != 0.0F
			   && this.ucomp > -2.0F)
		    this.dcomp
			-= 0.5 * (double) ((this.cd).airs
					   [this.cn]);
		if ((control).left) {
		    ...
		    f = ((float) -((this.cd).airc
				   [this.cn])
			 * this.m.cos((conto).xz) * (float) i);
		    f_11_ = ((float) -((this.cd).airc
				       [this.cn])
			     * this.m.sin((conto).xz)
			     * (float) i);
		} else if (this.lcomp > 0.0F)
		    this.lcomp -= 2.0F * ((this.cd)
						  .airs[this.cn]);
		if ((control).right) {
		    if (this.rcomp == 0.0F)
			this.rcomp = 5.0F;
		    ...
		    this.rcomp -= 2.0F * ((this.cd)
						  .airs[this.cn]);
		this.pzy += ((this.dcomp - this.ucomp)
				     * this.m.cos(this.pxy));
		if (bool)
		    (conto).xz
			+= ((this.dcomp - this.ucomp)
			    * this.m.sin(this.pxy));
		else
		    (conto).xz
			-= ((this.dcomp - this.ucomp)
	...
	if ((conto).wxz != 0 && !(control).left
		...
		if (Math.abs((conto).wxz)
		    < (this.cd).turn[this.cn] * 2)
		    (conto).wxz = 0;
		if ((conto).wxz > 0)
		    (conto).wxz
			-= ((this.cd).turn[this.cn]
	...
	int i_21_
	    = (int) (3600.0F / (this.speed * this.speed));
	if (i_21_ < 5)
	    i_21_ = 5;
	if (this.speed < 0.0F)
	    i_21_ = -i_21_;
	...
	for (int i_24_ = 0; i_24_ < 4; i_24_++) {
	    fs[i_24_]
		= (float) ((conto).keyx[i_24_] + (conto).x);
	    fs_23_[i_24_] = (float) (i_10_ + (conto).y);
	    fs_22_[i_24_]
		= (float) ((conto).z + (conto).keyz[i_24_]);
	    this.scy[i_24_] += 7.0F;
	}
	rot(fs, fs_23_, (conto).x, (conto).y, this.pxy,
	    4);
	rot(fs_23_, fs_22_, (conto).y, (conto).z,
	    this.pzy, 4);
	rot(fs, fs_22_, (conto).x, (conto).z,
	    (conto).xz, 4);
	boolean bool_25_ = false;
	double d = 0.0;
	int i_26_ = (int) ((this.scx[0] + this.scx[1]
			    + this.scx[2] + this.scx[3])
			   / 4.0F);
	int i_27_ = (int) ((this.scz[0] + this.scz[1]
			    + this.scz[2] + this.scz[3])
			   / 4.0F);
	...
	for (int i_29_ = 0; i_29_ < 4; i_29_++) {
	    ...
	    fs[i_29_] += (this.scx[0] + this.scx[1]
			  + this.scx[2] + this.scx[3]) / 4.0F;
	    fs_22_[i_29_]
		+= (this.scz[0] + this.scz[1]
		    + this.scz[2] + this.scz[3]) / 4.0F;
	}
	...
	int i_32_ = 1;
	for (int i_33_ = 0;
	     i_33_ < (trackers).sect[i_30_][i_31_].length;
	     i_33_++) {
	    int i_34_ = (trackers).sect[i_30_][i_31_][i_33_];
	    if (Math.abs((trackers).zy[i_34_]) != 90
		&& Math.abs((trackers).xy[i_34_]) != 90
		&& (Math.abs((conto).x
			     - (trackers).x[i_34_])
		    < (trackers).radx[i_34_])
		&& (Math.abs((conto).z
	...
	if (this.mtouch) {
	    ...
	    f_35_ -= ((float) Math.abs(this.txz - (conto).xz)
		      * this.speed / 250.0F);
	    if ((control).handb)
		f_35_
		    -= (float) (Math.abs(this.txz - (conto).xz)
				* 4);
	    ...
	    if (i_32_ == 2)
		f_35_ *= 0.55;
	    int i_36_ = -(int) (this.speed
				* this.m.sin((conto).xz)
				* this.m.cos(this.pzy));
	    int i_37_ = (int) (this.speed
			       * this.m.cos((conto).xz)
			       * this.m.cos(this.pzy));
	    int i_38_ = -(int) (this.speed
				* this.m.sin(this.pzy));
	    if (this.capsized || this.dest
		|| (checkpoints).haltall) {
		i_36_ = 0;
		i_37_ = 0;
		i_38_ = 0;
		f_35_ = ((this.cd).grip[this.cn]
			 / 5.0F);
		if (this.speed > 0.0F)
		    this.speed -= 2.0F;
		else
		...
		if (Math.abs(this.scx[i_41_] - (float) i_36_)
		    > f_35_) {
		...
		if (Math.abs(this.scz[i_41_] - (float) i_37_)
		    > f_35_) {
		...
		if (Math.abs(this.scy[i_41_] - (float) i_38_)
		    > f_35_) {
		...
		if (f_35_
		    < (this.cd).grip[this.cn]) {
		    if (this.txz != (conto).xz)
			this.dcnt++;
		    else if (this.dcnt != 0)
			this.dcnt = 0;
		    if (((float) this.dcnt
			 > 40.0F * f_35_ / ((this.cd).grip
					    [this.cn]))
			|| this.capsized) {
			float f_42_ = 1.0F;
			if (i_32_ != 0)
		...
		if (i_32_ == 3) {
		    ...
		    this.scy[i_43_]
			= (float) ((double) (-100.0F * this.m.random()
					     * (this.speed
						/ (float) ((
							    this.cd)
							   .swits
		...
		if (i_32_ == 4) {
		    ...
		    this.scy[i_44_]
			= (float) ((double) (-150.0F * this.m.random()
					     * (this.speed
						/ (float) ((
							    this.cd)
							   .swits
	...
	for (int i_49_ = 0; i_49_ < 4; i_49_++) {
		...
		float f_51_
		    = (Math.abs(this.m.sin(this.pxy))
		       + Math.abs(this.m.sin(this.pzy)));
		f_51_ /= 3.0F;
		if ((double) f_51_ > 0.4)
		    f_51_ = 0.4F;
	...
	int i_53_ = 0;
	for (int i_54_ = 0;
	     i_54_ < (trackers).sect[i_30_][i_31_].length;
	     i_54_++) {
		...
		if (bools_46_[i_58_]
		    && ((trackers).skd[i_55_] == 0
			|| (trackers).skd[i_55_] == 1)
		    && (fs[i_58_]
			> (float) ((trackers).x[i_55_]
				   - (trackers).radx[i_55_]))
		    && (fs[i_58_]
			< (float) ((trackers).x[i_55_]
				   + (trackers).radx[i_55_]))
		    && (fs_22_[i_58_]
			> (float) ((trackers).z[i_55_]
				   - (trackers).radz[i_55_]))
		    && (fs_22_[i_58_]
			< (float) ((trackers).z[i_55_]
				   + (trackers).radz[i_55_]))) {
		    conto.sprk(fs[i_58_], fs_23_[i_58_], fs_22_[i_58_],
			       ...
			       this.scz[i_58_], 1);
		    if (this.im == (this.xt).im)
			this.xt.gscrape((int) this.scx[i_58_],
						(int) this.scy[i_58_],
						(int) this.scz[i_58_]);
		}
		if (!bools[i_58_]
			...
			< (float) ((trackers).z[i_55_]
				   + (trackers).radz[i_55_]))
		    && (fs_23_[i_58_]
			> (float) ((trackers).y[i_55_]
				   - (trackers).rady[i_55_]))
		    && (fs_23_[i_58_]
			< (float) ((trackers).y[i_55_]
				   + (trackers).rady[i_55_]))
		    && (!(trackers).decor[i_55_]
			|| (this.m).resdown != 2
			|| (this.xt).multion != 0)) {
		    if ((trackers).xy[i_55_] == 0
			&& (trackers).zy[i_55_] == 0
			&& (trackers).y[i_55_] != 250
			&& (fs_23_[i_58_]
			    > (float) ((trackers).y[i_55_] - 5))) {
			i_57_++;
			this.wtouch = true;
			this.gtouch = true;
			...
			if ((double) f_60_ > 0.4)
			    f_60_ = 0.4F;
			f_60_ += ((this.cd).bounce
				  [this.cn]);
			if ((double) f_60_ < 1.1)
			    f_60_ = 1.1F;
			regy(i_58_, Math.abs(this.scy[i_58_] * f_60_),
			     conto);
			if (this.scy[i_58_] > 0.0F)
			    this.scy[i_58_]
				-= Math.abs(this.scy[i_58_] * f_60_);
			bools[i_58_] = true;
		    }
		    if ((trackers).zy[i_55_] == -90
			&& (fs_22_[i_58_]
			    < (float) ((trackers).z[i_55_]
				       + (trackers).radz[i_55_]))
			&& (this.scz[i_58_] < 0.0F
			    || (trackers).radz[i_55_] == 287)) {
			...
			if (bool_25_)
			    f_62_ = 0.0F;
			f_62_ += (double) ((this.cd).bounce
					   [this.cn]) - 0.2;
			if ((double) f_62_ < 1.1)
			    f_62_ = 1.1F;
			regz(i_58_, Math.abs(this.scz[i_58_] * f_62_
					     * (float) ((trackers)
							.dam[i_55_])), conto);
			this.scz[i_58_]
			    += Math.abs(this.scz[i_58_] * f_62_);
			this.skid = 2;
			bool_6_ = true;
			bools[i_58_] = true;
			...
			if (bool_25_)
			    f_64_ = 0.0F;
			f_64_ += (double) ((this.cd).bounce
					   [this.cn]) - 0.2;
			if ((double) f_64_ < 1.1)
			    f_64_ = 1.1F;
			regz(i_58_, -Math.abs(this.scz[i_58_] * f_64_
					      * (float) ((trackers)
							 .dam[i_55_])), conto);
			this.scz[i_58_]
			    -= Math.abs(this.scz[i_58_] * f_64_);
			this.skid = 2;
			bool_6_ = true;
			bools[i_58_] = true;
			if (!(trackers).notwall[i_55_])
		    ...
		    if ((trackers).xy[i_55_] == -90
			&& (fs[i_58_]
			    < (float) ((trackers).x[i_55_]
				       + (trackers).radx[i_55_]))
			&& (this.scx[i_58_] < 0.0F
			    || (trackers).radx[i_55_] == 287)) {
			...
			if (bool_25_)
			    f_66_ = 0.0F;
			f_66_ += (double) ((this.cd).bounce
					   [this.cn]) - 0.2;
			if ((double) f_66_ < 1.1)
			    f_66_ = 1.1F;
			regx(i_58_, Math.abs(this.scx[i_58_] * f_66_
					     * (float) ((trackers)
							.dam[i_55_])), conto);
			this.scx[i_58_]
			    += Math.abs(this.scx[i_58_] * f_66_);
			this.skid = 2;
			bool_6_ = true;
			bools[i_58_] = true;
			if (!(trackers).notwall[i_55_])
			...
			if (bool_25_)
			    f_68_ = 0.0F;
			f_68_ += (double) ((this.cd).bounce
					   [this.cn]) - 0.2;
			if ((double) f_68_ < 1.1)
			    f_68_ = 1.1F;
			regx(i_58_, -Math.abs(this.scx[i_58_] * f_68_
					      * (float) ((trackers)
							 .dam[i_55_])), conto);
			this.scx[i_58_]
			    -= Math.abs(this.scx[i_58_] * f_68_);
			this.skid = 2;
			bool_6_ = true;
			bools[i_58_] = true;
			if (!(trackers).notwall[i_55_])
		    ...
		    if ((trackers).zy[i_55_] != 0
			&& (trackers).zy[i_55_] != 90
			&& (trackers).zy[i_55_] != -90) {
			int i_69_ = 90 + (trackers).zy[i_55_];
			float f_70_
			    = (1.0F
			       + (float) (50 - Math.abs((trackers)
							.zy[i_55_])) / 30.0F);
			if (f_70_ < 1.0F)
			    f_70_ = 1.0F;
			float f_71_
			...
			float f_72_
			    = ((float) (trackers).z[i_55_]
				  ...
				  + ((fs_22_[i_58_]
				      - (float) (trackers).z[i_55_])
				     * this.m.cos(i_69_))));
			if (f_72_ > (float) (trackers).z[i_55_]
			...
			float f_77_
			    = ((float) (trackers).x[i_55_]
				  ...
				  + ((fs[i_58_]
				      - (float) (trackers).x[i_55_])
				     * this.m.cos(i_74_))));
			if (f_77_ > (float) (trackers).x[i_55_]
	...
	for (int i_79_ = 0; i_79_ < 4; i_79_++) {
		...
		if (this.crank[i_79_][i_80_]
		    == this.lcrank[i_79_][i_80_])
		    this.crank[i_79_][i_80_] = 0;
		this.lcrank[i_79_][i_80_]
		    = this.crank[i_79_][i_80_];
	    }
	}
	int i_81_ = 0;
	int i_82_ = 0;
	...
	if (this.scy[2] != this.scy[0]) {
	    ...
	    d = (Math.sqrt((double) ((fs_22_[0] - fs_22_[2]) * (fs_22_[0]
				     ...
				     + (fs_23_[0] - fs_23_[2]) * (fs_23_[0]
								  - fs_23_[2])
				     + (fs[0] - fs[2]) * (fs[0] - fs[2])))
		 / (double) (Math.abs((conto).keyz[0])
			     + Math.abs((conto).keyz[2])));
	    if (d >= 0.9998)
		i_81_ = i;
	    else
		i_81_
	...
	if (this.scy[3] != this.scy[1]) {
	    ...
	    d = (Math.sqrt((double) ((fs_22_[1] - fs_22_[3]) * (fs_22_[1]
				     ...
				     + (fs_23_[1] - fs_23_[3]) * (fs_23_[1]
								  - fs_23_[3])
				     + (fs[1] - fs[3]) * (fs[1] - fs[3])))
		 / (double) (Math.abs((conto).keyz[1])
			     + Math.abs((conto).keyz[3])));
	    if (d >= 0.9998)
		i_82_ = i;
	    else
		i_82_
	...
	if (this.scy[1] != this.scy[0]) {
	    ...
	    d = (Math.sqrt((double) ((fs_22_[0] - fs_22_[1]) * (fs_22_[0]
				     ...
				     + (fs_23_[0] - fs_23_[1]) * (fs_23_[0]
								  - fs_23_[1])
				     + (fs[0] - fs[1]) * (fs[0] - fs[1])))
		 / (double) (Math.abs((conto).keyx[0])
			     + Math.abs((conto).keyx[1])));
	    if (d >= 0.9998)
		i_83_ = i;
	    else
		i_83_
	...
	if (this.scy[3] != this.scy[2]) {
	    ...
	    d = (Math.sqrt((double) ((fs_22_[2] - fs_22_[3]) * (fs_22_[2]
				     ...
				     + (fs_23_[2] - fs_23_[3]) * (fs_23_[2]
								  - fs_23_[3])
				     + (fs[2] - fs[3]) * (fs[2] - fs[3])))
		 / (double) (Math.abs((conto).keyx[2])
			     + Math.abs((conto).keyx[3])));
	    if (d >= 0.9998)
		i_84_ = i;
	    else
		i_84_
	...
	if (bool_6_) {
	    int i_85_;
	    for (i_85_ = Math.abs((conto).xz + 45); i_85_ > 180;
		 i_85_ -= 360) {
		/* empty */
	    }
	...
	(conto).xz
	    += (this.forca
		* (this.scz[0] * (float) this.nmlt
		   - this.scz[1] * (float) this.pmlt
		   + this.scz[2] * (float) this.pmlt
		   - this.scz[3] * (float) this.nmlt
		   + this.scx[0] * (float) this.pmlt
		   + this.scx[1] * (float) this.nmlt
		   - this.scx[2] * (float) this.nmlt
		   - this.scx[3] * (float) this.pmlt));
	...
	(conto).y
	    = (int) ((fs_23_[0] + fs_23_[1] + fs_23_[2] + fs_23_[3]) / 4.0F
		     - ((float) i_10_ * this.m.cos(this.pzy)
			* this.m.cos(this.pxy))
		     + f_12_);
	if (bool)
	    i = -1;
	...
	(conto).x
	    = (int) ((fs[0]
		      - ((float) (conto).keyx[0]
			 * this.m.cos((conto).xz))
		      + ((float) (i * (conto).keyz[0])
			 * this.m.sin((conto).xz))
		      + fs[1]
		      - ((float) (conto).keyx[1]
			 * this.m.cos((conto).xz))
		      + ((float) (i * (conto).keyz[1])
			 * this.m.sin((conto).xz))
		      + fs[2]
		      - ((float) (conto).keyx[2]
			 * this.m.cos((conto).xz))
		      + ((float) (i * (conto).keyz[2])
			 * this.m.sin((conto).xz))
		      + fs[3]
		      - ((float) (conto).keyx[3]
			 * this.m.cos((conto).xz))
		      + ((float) (i * (conto).keyz[3])
			 * this.m.sin((conto).xz))) / 4.0F
		     + ((float) i_10_ * this.m.sin(this.pxy)
			* this.m.cos((conto).xz))
		     - ((float) i_10_ * this.m.sin(this.pzy)
			* this.m.sin((conto).xz))
		     + f);
	(conto).z
	    = (int) ((fs_22_[0]
	...
	if (Math.abs(this.speed) > 10.0F || !this.mtouch) {
		...
		if (this.pxy > (conto).xy)
		    (conto).xy
			+= 2 + (this.pxy - (conto).xy) / 2;
		else
		    (conto).xy
			-= 2 + ((conto).xy - this.pxy) / 2;
	    } else
		(conto).xy = this.pxy;
	    if (Math.abs(this.pzy - (conto).zy) >= 4) {
		if (this.pzy > (conto).zy)
		    (conto).zy
			+= 2 + (this.pzy - (conto).zy) / 2;
		else
		    (conto).zy
			-= 2 + ((conto).zy - this.pzy) / 2;
	    } else
		(conto).zy = this.pzy;
	}
	if (this.wtouch && !this.capsized) {
	    float f_87_
		= (float) ((double) (this.speed
				     / (float) ((this.cd)
						.swits[this.cn][2])
				     * 14.0F)
			   * ((double) ((this.cd).bounce
					[this.cn])
			      - 0.4));
	    if ((control).left && this.tilt < f_87_
		&& this.tilt >= 0.0F)
		this.tilt += 0.4;
	    else if ((control).right && this.tilt > -f_87_
		     && this.tilt <= 0.0F)
		this.tilt -= 0.4;
	    else if ((double) Math.abs(this.tilt)
		     > 3.0 * ((double) ((this.cd).bounce
					[this.cn])
			      - 0.4)) {
		if (this.tilt > 0.0F)
		    this.tilt
	...
	for (int i_92_ = 0; i_92_ < (checkpoints).n; i_92_++) {
		...
		if ((checkpoints).typ[i_92_] == 1) {
			...
			== i_91_ + (this.nlaps
				    * (checkpoints).nsp))
			i_4_ = 1;
		    if (((float) Math.abs((conto).z
					  - ((checkpoints).z
					     [i_92_]))
			 < 60.0F + Math.abs(this.scz[0]
					    + this.scz[1]
					    + this.scz[2]
					    + this.scz[3]) / 4.0F)
			&& (Math.abs((conto).x
		...
		if ((checkpoints).typ[i_92_] == 2) {
			...
			== i_91_ + (this.nlaps
				    * (checkpoints).nsp))
			i_4_ = 1;
		    if (((float) Math.abs((conto).x
					  - ((checkpoints).x
					     [i_92_]))
			 < 60.0F + Math.abs(this.scx[0]
					    + this.scx[1]
					    + this.scx[2]
					    + this.scx[3]) / 4.0F)
			&& (Math.abs((conto).z
	    ...
	    if (py((conto).x / 100,
		   (checkpoints).x[i_92_] / 100,
		   (conto).z / 100,
		   (checkpoints).z[i_92_] / 100) * i_4_ < i_90_
		|| i_90_ == 0) {
		i_89_ = i_92_;
		i_90_ = py((conto).x / 100,
			   (checkpoints).x[i_92_] / 100,
			   (conto).z / 100,
			   (checkpoints).z[i_92_] / 100) * i_4_;
	    }
	}
	if (this.clear
	    == i_91_ + this.nlaps * (checkpoints).nsp) {
	    this.nlaps++;
	    if ((this.xt).multion == 1
		&& this.im == (this.xt).im) {
		if (((this.xt).laptime
		     < (this.xt).fastestlap)
		    || (this.xt).fastestlap == 0)
		    (this.xt).fastestlap
			= (this.xt).laptime;
		(this.xt).laptime = 0;
	    }
	...
	if (this.im == (this.xt).im) {
		...
		&& (this.xt).starcnt == 0)
		(this.xt).laptime++;
	    for ((this.m).checkpoint = this.clear;
		 ((this.m).checkpoint
		  >= (checkpoints).nsp);
		 (this.m).checkpoint
		     -= (checkpoints).nsp) {
	    ...
	    if (this.clear == ((checkpoints).nlaps
				       * (checkpoints).nsp) - 1)
	...
	if (this.focus == -1) {
	    ...
	    i_89_ = this.focus;
	    if (this.im == (this.xt).im) {
		if (this.missedcp == 0 && this.mtouch
		    && (Math.sqrt((double) py((conto).x / 10,
					      ((checkpoints).x
					       [this.focus]) / 10,
					      (conto).z / 10,
					      ((checkpoints).z
	...
	if (this.fixes != 0) {
	    if ((this.m).noelec == 0) {
		for (int i_93_ = 0; i_93_ < (checkpoints).fn;
		     i_93_++) {
		    if (!(checkpoints).roted[i_93_]) {
			if ((Math.abs((conto).z
				      - (checkpoints).fz[i_93_])
			     < 200)
			    && (py((conto).x / 100,
				   (checkpoints).fx[i_93_] / 100,
				   (conto).y / 100,
				   (checkpoints).fy[i_93_] / 100)
				< 30)) {
			    if ((conto).dist == 0)
				(conto).fcnt = 8;
			    else {
			    ...
			    (this.rpd).fix[this.im]
				= 300;
			}
		    } else if (Math.abs((conto).x
					- ((checkpoints).fx
					   [i_93_])) < 200
			       && py((conto).z / 100,
				     ((checkpoints).fz[i_93_]
	    ...
	    for (int i_94_ = 0; i_94_ < (checkpoints).fn;
		 i_94_++) {
		if (rpy((float) ((conto).x / 100),
			(float) ((checkpoints).fx[i_94_] / 100),
			(float) ((conto).y / 100),
			(float) ((checkpoints).fy[i_94_] / 100),
			(float) ((conto).z / 100),
			(float) ((checkpoints).fz[i_94_] / 100))
		    < 760)
		    (this.m).noelec = 2;
	    }
	}
	...
	if (!this.mtouch) {
		...
		if (this.gtouch && this.trcnt != 0) {
			...
			if (Math.abs(this.travxy) > 90)
			    this.powerup
				+= ((float) Math.abs(this.travxy)
				    / 24.0F);
			else if (this.rtab)
			    this.powerup += 30.0F;
			if (Math.abs(this.travzy) > 90)
			    this.powerup
				+= ((float) Math.abs(this.travzy)
				    / 18.0F);
			else {
			    if (this.ftab)
				this.powerup += 40.0F;
		...
		if (this.capcnt == 0) {
			...
			if (Math.abs(this.scz[i_96_]) < 70.0F
			    && Math.abs(this.scx[i_96_]) < 70.0F)
			    i_95_++;
		    }
		    if (i_95_ == 4)
			this.capcnt = 1;
	...
	if (this.im == (this.xt).im
		...
		if ((this.rpd).hcaught)
		    (this.rpd).whenwasted
			= (int) (185.0F + this.m.random() * 20.0F);
	    }
	}
    }
	...
	if (f > 100.0F) {
	    ...
	    if (i_99_ * i_98_ == 0)
		this.shakedam
		    = (int) ((Math.abs(f) + (float) this.shakedam)
			     / 2.0F);
	    if (this.im == (this.xt).im
		|| this.colidim)
		this.xt.crash(f, i_99_ * i_98_);
	    if (i_99_ * i_98_ == 0 || this.mtouch) {
		for (int i_102_ = 0; i_102_ < (conto).npl; i_102_++) {
		    float f_103_ = 0.0F;
		    for (int i_104_ = 0;
			 i_104_ < ((conto).p[i_102_]).n;
			 i_104_++) {
			if (((conto).p[i_102_]).wz == 0
			    && (py((conto).keyx[i],
				   (((conto).p[i_102_]).ox
				    [i_104_]),
				   (conto).keyz[i],
				...
				< ((this.cd).clrad
				   [this.cn]))) {
			    f_103_ = f / 20.0F * this.m.random();
			    ((conto).p[i_102_]).oz[i_104_]
				+= f_103_ * this.m.sin(i_100_);
			    ((conto).p[i_102_]).ox[i_104_]
				-= f_103_ * this.m.sin(i_101_);
			    if (bool) {
				this.hitmag += Math.abs(f_103_);
			...
			if (!((conto).p[i_102_]).nocol
			    && (((conto).p[i_102_]).glass
				!= 1)) {
			    if (((conto).p[i_102_]).bfase > 20
				&& (double) (((conto).p
						      [i_102_])).hsb[1] > 0.25)
				((conto).p[i_102_]).hsb[1]
				    = 0.25F;
			    if (((conto).p[i_102_]).bfase > 25
				&& (double) (((conto).p
						      [i_102_])).hsb[2] > 0.7)
				((conto).p[i_102_]).hsb[2]
				    = 0.7F;
			    if (((conto).p[i_102_]).bfase > 30
				&& (double) (((conto).p
						      [i_102_])).hsb[1] > 0.15)
				((conto).p[i_102_]).hsb[1]
				    = 0.15F;
			    if (((conto).p[i_102_]).bfase > 35
				&& (double) (((conto).p
						      [i_102_])).hsb[2] > 0.6)
				((conto).p[i_102_]).hsb[2]
				    = 0.6F;
			    if (((conto).p[i_102_]).bfase > 40)
				((conto).p[i_102_]).hsb[0]
				    = 0.075F;
			    if (((conto).p[i_102_]).bfase > 50
				&& (double) (((conto).p
						      [i_102_])).hsb[2] > 0.5)
				((conto).p[i_102_]).hsb[2]
				    = 0.5F;
				...
				((conto).p[i_102_]).hsb[0]
				    = 0.05F;
			    ((conto).p[i_102_]).bfase
				+= f_103_;
			    new Color(((conto).p[i_102_]).c[0],
				      ((conto).p[i_102_]).c[1],
				      (((conto).p[i_102_]).c
				       [2]));
			    Color color
		...
		if (this.nbsq > 0) {
		    int i_105_ = 0;
		    int i_106_ = 1;
		    for (int i_107_ = 0; i_107_ < (conto).npl;
			 i_107_++) {
			float f_108_ = 0.0F;
			for (int i_109_ = 0;
			     i_109_ < ((conto).p[i_107_]).n;
			     i_109_++) {
			    if (((conto).p[i_107_]).wz == 0) {
				f_108_ = f / 15.0F * this.m.random();
				if (((Math.abs((((conto).p
							 [i_107_])).oy[i_109_]
					       - ((this.cd)
						  .flipy[this.cn])
	...
	if (Math.abs(f) > 100.0F) {
		...
		f += 100.0F;
	    this.shakedam
		= (int) ((Math.abs(f) + (float) this.shakedam) / 2.0F);
	    if (this.im == (this.xt).im
		|| this.colidim)
		this.xt.crash(f, 0);
		...
		float f_112_ = 0.0F;
		for (int i_113_ = 0;
		     i_113_ < ((conto).p[i_111_]).n;
		     i_113_++) {
		    if (((conto).p[i_111_]).wz == 0
			&& (py((conto).keyx[i],
			       ((conto).p[i_111_]).ox[i_113_],
			       (conto).keyz[i],
			       ((conto).p[i_111_]).oz[i_113_])
			    < ((this.cd).clrad
			       [this.cn]))) {
			f_112_ = f / 20.0F * this.m.random();
			((conto).p[i_111_]).oz[i_113_]
			    -= (f_112_ * this.m.sin((conto).xz)
				* this.m.cos((conto).zy));
			((conto).p[i_111_]).ox[i_113_]
			    += (f_112_ * this.m.cos((conto).xz)
				* this.m.cos((conto).xy));
			if (bool) {
			    this.hitmag += Math.abs(f_112_);
		...
		if (f_112_ != 0.0F) {
		    ...
		    if (!((conto).p[i_111_]).nocol
			&& ((conto).p[i_111_]).glass != 1) {
			if (((conto).p[i_111_]).bfase > 20
			    && (double) (((conto).p[i_111_])
					 .hsb[1]) > 0.25)
			    ((conto).p[i_111_]).hsb[1] = 0.25F;
			if (((conto).p[i_111_]).bfase > 25
			    && (double) (((conto).p[i_111_])
					 .hsb[2]) > 0.7)
			    ((conto).p[i_111_]).hsb[2] = 0.7F;
			if (((conto).p[i_111_]).bfase > 30
			    && (double) (((conto).p[i_111_])
					 .hsb[1]) > 0.15)
			    ((conto).p[i_111_]).hsb[1] = 0.15F;
			if (((conto).p[i_111_]).bfase > 35
			    && (double) (((conto).p[i_111_])
					 .hsb[2]) > 0.6)
			    ((conto).p[i_111_]).hsb[2] = 0.6F;
			if (((conto).p[i_111_]).bfase > 40)
			    ((conto).p[i_111_]).hsb[0]
				= 0.075F;
			if (((conto).p[i_111_]).bfase > 50
			    && (double) (((conto).p[i_111_])
					 .hsb[2]) > 0.5)
			    ((conto).p[i_111_]).hsb[2] = 0.5F;
			if (((conto).p[i_111_]).bfase > 60)
			    ((conto).p[i_111_]).hsb[0] = 0.05F;
			((conto).p[i_111_]).bfase
			    += Math.abs(f_112_);
			new Color(((conto).p[i_111_]).c[0],
				  ((conto).p[i_111_]).c[1],
				  ((conto).p[i_111_]).c[2]);
			Color color
			    = (Color.getHSBColor
			       (((conto).p[i_111_]).hsb[0],
				((conto).p[i_111_]).hsb[1],
	...
	if (Math.abs(f) > 100.0F) {
		...
		float f_116_ = 0.0F;
		for (int i_117_ = 0;
		     i_117_ < ((conto).p[i_115_]).n;
		     i_117_++) {
		    if (((conto).p[i_115_]).wz == 0
			&& (py((conto).keyx[i],
			       ((conto).p[i_115_]).ox[i_117_],
			       (conto).keyz[i],
			       ((conto).p[i_115_]).oz[i_117_])
			    < ((this.cd).clrad
			       [this.cn]))) {
			f_116_ = f / 20.0F * this.m.random();
			((conto).p[i_115_]).oz[i_117_]
			    += (f_116_ * this.m.cos((conto).xz)
				* this.m.cos((conto).zy));
			((conto).p[i_115_]).ox[i_117_]
			    += (f_116_ * this.m.sin((conto).xz)
				* this.m.cos((conto).xy));
			if (bool) {
			    this.hitmag += Math.abs(f_116_);
		...
		if (f_116_ != 0.0F) {
		    ...
		    if (!((conto).p[i_115_]).nocol
			&& ((conto).p[i_115_]).glass != 1) {
			if (((conto).p[i_115_]).bfase > 20
			    && (double) (((conto).p[i_115_])
					 .hsb[1]) > 0.25)
			    ((conto).p[i_115_]).hsb[1] = 0.25F;
			if (((conto).p[i_115_]).bfase > 25
			    && (double) (((conto).p[i_115_])
					 .hsb[2]) > 0.7)
			    ((conto).p[i_115_]).hsb[2] = 0.7F;
			if (((conto).p[i_115_]).bfase > 30
			    && (double) (((conto).p[i_115_])
					 .hsb[1]) > 0.15)
			    ((conto).p[i_115_]).hsb[1] = 0.15F;
			if (((conto).p[i_115_]).bfase > 35
			    && (double) (((conto).p[i_115_])
					 .hsb[2]) > 0.6)
			    ((conto).p[i_115_]).hsb[2] = 0.6F;
			if (((conto).p[i_115_]).bfase > 40)
			    ((conto).p[i_115_]).hsb[0]
				= 0.075F;
			if (((conto).p[i_115_]).bfase > 50
			    && (double) (((conto).p[i_115_])
					 .hsb[2]) > 0.5)
			    ((conto).p[i_115_]).hsb[2] = 0.5F;
			if (((conto).p[i_115_]).bfase > 60)
			    ((conto).p[i_115_]).hsb[0] = 0.05F;
			((conto).p[i_115_]).bfase
			    += Math.abs(f_116_);
			new Color(((conto).p[i_115_]).c[0],
				  ((conto).p[i_115_]).c[1],
				  ((conto).p[i_115_]).c[2]);
			Color color
			    = (Color.getHSBColor
			       (((conto).p[i_115_]).hsb[0],
				((conto).p[i_115_]).hsb[1],
	...
	for (int i = 0; i < 4; i++) {
	    ...
	    if (this.capsized)
		fs_120_[i] = (float) ((conto).y
				      + ((this.cd).flipy
					 [this.cn])
				      + this.squash);
	    else
		fs_120_[i]
		    = (float) ((conto).y + (conto).grat);
	    fs_121_[i] = (float) ((conto).z + (conto).keyz[i]);
	    fs_122_[i] = (float) ((conto_119_).x
				  + (conto_119_).keyx[i]);
	    if (this.capsized)
		fs_123_[i] = (float) ((conto_119_).y
				      + ((this.cd).flipy
					 [(mad_118_).cn])
				      + (mad_118_).squash);
	    else
		fs_123_[i] = (float) ((conto_119_).y
				      + (conto_119_).grat);
	    fs_124_[i] = (float) ((conto_119_).z
				  + (conto_119_).keyz[i]);
	}
	rot(fs, fs_120_, (conto).x, (conto).y,
	    (conto).xy, 4);
	rot(fs_120_, fs_121_, (conto).y, (conto).z,
	    (conto).zy, 4);
	rot(fs, fs_121_, (conto).x, (conto).z,
	    (conto).xz, 4);
	rot(fs_122_, fs_123_, (conto_119_).x, (conto_119_).y,
	    (conto_119_).xy, 4);
	rot(fs_123_, fs_124_, (conto_119_).y, (conto_119_).z,
	    (conto_119_).zy, 4);
	rot(fs_122_, fs_124_, (conto_119_).x, (conto_119_).z,
	    (conto_119_).xz, 4);
	if ((double) rpy((float) (conto).x,
			 (float) (conto_119_).x,
			 (float) (conto).y,
			 (float) (conto_119_).y,
			 (float) (conto).z,
			 (float) (conto_119_).z)
	    < (double) ((conto).maxR * (conto).maxR
			+ ((conto_119_).maxR
			   * (conto_119_).maxR)) * 1.5) {
	    if (!this.caught[(mad_118_).im]
		&& (this.speed != 0.0F
		    || (mad_118_).speed != 0.0F)) {
		if (Math.abs(this.power * this.speed
			     * ((this.cd).moment
				[this.cn]))
		    != Math.abs((mad_118_).power * (mad_118_).speed
				* ((this.cd).moment
				   [(mad_118_).cn]))) {
		    if (Math.abs(this.power * this.speed
				 * ((this.cd).moment
				    [this.cn]))
			> Math.abs((mad_118_).power
				   * (mad_118_).speed
				   * ((this.cd).moment
				      [(mad_118_).cn])))
			this.dominate[(mad_118_).im] = true;
		    else
			this.dominate[(mad_118_).im] = false;
		} else if (((this.cd).moment
			    [this.cn])
			   > ((this.cd).moment
			      [(mad_118_).cn]))
		    this.dominate[(mad_118_).im] = true;
		    ...
		    this.dominate[(mad_118_).im] = false;
		this.caught[(mad_118_).im] = true;
	    }
	} else if (this.caught[(mad_118_).im])
	    this.caught[(mad_118_).im] = false;
	int i = 0;
	int i_125_ = 0;
	if (this.dominate[(mad_118_).im]) {
	    int i_126_
		= (int) ((((this.scz[0] - (mad_118_).scz[0]
			    + this.scz[1] - (mad_118_).scz[1]
			    + this.scz[2] - (mad_118_).scz[2]
			    + this.scz[3] - (mad_118_).scz[3])
			   * (this.scz[0] - (mad_118_).scz[0]
			      + this.scz[1] - (mad_118_).scz[1]
			      + this.scz[2] - (mad_118_).scz[2]
			      + this.scz[3] - (mad_118_).scz[3]))
			  + ((this.scx[0] - (mad_118_).scx[0]
			      + this.scx[1] - (mad_118_).scx[1]
			      + this.scx[2] - (mad_118_).scx[2]
			      + this.scx[3] - (mad_118_).scx[3])
			     * (this.scx[0] - (mad_118_).scx[0]
				+ this.scx[1] - (mad_118_).scx[1]
				+ this.scx[2] - (mad_118_).scx[2]
				+ this.scx[3]
		...
		for (int i_129_ = 0; i_129_ < 4; i_129_++) {
			...
			< ((float) (i_126_ + i_127_)
			   * (((this.cd).comprad
			      ...
			      + ((this.cd).comprad
				 [this.cn])))) {
			if (Math.abs(this.scx[i_128_]
				     * ((this.cd).moment
					[this.cn]))
			    > Math.abs((mad_118_).scx[i_129_]
				       * ((this.cd).moment
					  [(mad_118_).cn]))) {
			    float f_130_
				= ((mad_118_).scx[i_129_]
			...
			if (Math.abs(this.scz[i_128_]
				     * ((this.cd).moment
					[this.cn]))
			    > Math.abs((mad_118_).scz[i_129_]
				       * ((this.cd).moment
					  [(mad_118_).cn]))) {
			    float f_132_
				= ((mad_118_).scz[i_129_]
	...
	if ((this.xt).multion == 1) {
	    if ((mad_118_).im == (this.xt).im
		&& i != 0)
		(this.xt).dcrashes[this.im] += i;
	    if (this.im == (this.xt).im
		&& i_125_ != 0)
		(this.xt).dcrashes[(mad_118_).im]
		    += i_125_;
	}
    }
	...
	if (i_136_ != 0) {
		...
		fs[i_138_]
		    = (float) i + ((f - (float) i) * this.m.cos(i_136_)
				   - ((f_139_ - (float) i_135_)
				      * this.m.sin(i_136_)));
		fs_134_[i_138_]
		    = ((float) i_135_
		       + ((f - (float) i) * this.m.sin(i_136_)
			  + ((f_139_ - (float) i_135_)
			     * this.m.cos(i_136_))));
	    }
	}
