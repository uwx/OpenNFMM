...
public class Mad
{
	...
	rot(fs, fs_23_, (conto).x, (conto).y, pxy,
	    4);
	...
	double d = 0.0;
	int i_26_ = (int) ((scx[0] + scx[1]
			    + scx[2] + scx[3])
			   / 4.0F);
	int i_27_ = (int) ((scz[0] + scz[1]
			    + scz[2] + scz[3])
			   / 4.0F);
	...
	if (mtouch) {
	    ...
	    f_35_ -= ((float) Math.abs(txz - (conto).xz)
		      * speed / 250.0F);
	    if ((control).handb)
		f_35_
		    -= (float) (Math.abs(txz - (conto).xz)
				* 4);
	    if (f_35_ < (cd).grip[cn]) {
		if (skid != 2)
		    skid = 1;
		...
		if (Math.abs(scx[i_41_] - (float) i_36_)
		    > f_35_) {
		...
		if (Math.abs(scz[i_41_] - (float) i_37_)
		    > f_35_) {
		...
		if (Math.abs(scy[i_41_] - (float) i_38_)
		    > f_35_) {
	...
	for (int i_54_ = 0;
		...
		if (!bools[i_58_]
			...
			if ((double) f_60_ < 1.1)
			    f_60_ = 1.1F;
			regy(i_58_, Math.abs(scy[i_58_] * f_60_),
			     conto);
			if (scy[i_58_] > 0.0F)
			    scy[i_58_]
				-= Math.abs(scy[i_58_] * f_60_);
			bools[i_58_] = true;
		    }
			...
			if (bool_25_)
			    f_62_ = 0.0F;
			f_62_ += (double) ((cd).bounce
					   [cn]) - 0.2;
			if ((double) f_62_ < 1.1)
			    f_62_ = 1.1F;
			regz(i_58_, Math.abs(scz[i_58_] * f_62_
					     * (float) ((trackers)
							.dam[i_55_])), conto);
			scz[i_58_]
			    += Math.abs(scz[i_58_] * f_62_);
			skid = 2;
			bool_6_ = true;
			bools[i_58_] = true;
			...
			if (bool_25_)
			    f_64_ = 0.0F;
			f_64_ += (double) ((cd).bounce
					   [cn]) - 0.2;
			if ((double) f_64_ < 1.1)
			    f_64_ = 1.1F;
			regz(i_58_, -Math.abs(scz[i_58_] * f_64_
					      * (float) ((trackers)
							 .dam[i_55_])), conto);
			scz[i_58_]
			    -= Math.abs(scz[i_58_] * f_64_);
			skid = 2;
			bool_6_ = true;
			bools[i_58_] = true;
			if (!(trackers).notwall[i_55_])
			...
			if (bool_25_)
			    f_66_ = 0.0F;
			f_66_ += (double) ((cd).bounce
					   [cn]) - 0.2;
			if ((double) f_66_ < 1.1)
			    f_66_ = 1.1F;
			regx(i_58_, Math.abs(scx[i_58_] * f_66_
					     * (float) ((trackers)
							.dam[i_55_])), conto);
			scx[i_58_]
			    += Math.abs(scx[i_58_] * f_66_);
			skid = 2;
			bool_6_ = true;
			bools[i_58_] = true;
			if (!(trackers).notwall[i_55_])
			...
			if (bool_25_)
			    f_68_ = 0.0F;
			f_68_ += (double) ((cd).bounce
					   [cn]) - 0.2;
			if ((double) f_68_ < 1.1)
			    f_68_ = 1.1F;
			regx(i_58_, -Math.abs(scx[i_58_] * f_68_
					      * (float) ((trackers)
							 .dam[i_55_])), conto);
			scx[i_58_]
			    -= Math.abs(scx[i_58_] * f_68_);
			skid = 2;
			bool_6_ = true;
			bools[i_58_] = true;
			if (!(trackers).notwall[i_55_])
	...
	(conto).x
	    = (int) ((fs[0]
			 ...
			 * m.sin((conto).xz))) / 4.0F
		     + ((float) i_10_ * m.sin(pxy)
			* m.cos((conto).xz))
		     - ((float) i_10_ * m.sin(pzy)
			* m.sin((conto).xz))
		     + f);
	(conto).z
	    = (int) ((fs_22_[0]
	...
	if (wtouch && !capsized) {
		...
		= (float) ((double) (speed
			   ...
			   * ((double) ((cd).bounce
					[cn])
			      - 0.4));
	    if ((control).left && tilt < f_87_
		&& tilt >= 0.0F)
		tilt += 0.4;
	    else if ((control).right && tilt > -f_87_
		     && tilt <= 0.0F)
		tilt -= 0.4;
	    else if ((double) Math.abs(tilt)
		     > 3.0 * ((double) ((cd).bounce
					[cn])
			      - 0.4)) {
		if (tilt > 0.0F)
		    tilt
	...
	if (!mtouch) {
		...
		if (gtouch && trcnt != 0) {
			...
			if (Math.abs(travxy) > 90)
			    powerup
				+= ((float) Math.abs(travxy)
				    / 24.0F);
			else if (rtab)
			    powerup += 30.0F;
			if (Math.abs(travzy) > 90)
			    powerup
				+= ((float) Math.abs(travzy)
				    / 18.0F);
			else {
			    if (ftab)
				powerup += 40.0F;
		...
		if (capcnt == 0) {
			...
			if (Math.abs(scz[i_96_]) < 70.0F
			    && Math.abs(scx[i_96_]) < 70.0F)
			    i_95_++;
		    }
		    if (i_95_ == 4)
			capcnt = 1;
	...
	if (Math.abs(f) > 100.0F) {
		...
		f += 100.0F;
	    shakedam
		= (int) ((Math.abs(f) + (float) shakedam) / 2.0F);
	    if (im == (xt).im
		|| colidim)
		xt.crash(f, 0);
		...
		for (int i_113_ = 0;
			...
			((conto).p[i_111_]).oz[i_113_]
			    -= (f_112_ * m.sin((conto).xz)
				* m.cos((conto).zy));
			((conto).p[i_111_]).ox[i_113_]
			    += (f_112_ * m.cos((conto).xz)
				* m.cos((conto).xy));
			if (bool) {
	...
	if (Math.abs(f) > 100.0F) {
		...
		for (int i_117_ = 0;
			...
			((conto).p[i_115_]).oz[i_117_]
			    += (f_116_ * m.cos((conto).xz)
				* m.cos((conto).zy));
			((conto).p[i_115_]).ox[i_117_]
			    += (f_116_ * m.sin((conto).xz)
				* m.cos((conto).xy));
			if (bool) {
	...
	if (dominate[(mad_118_).im]) {
	    int i_126_
		= (int) ((((scz[0] - (mad_118_).scz[0]
			    + scz[1] - (mad_118_).scz[1]
			    + scz[2] - (mad_118_).scz[2]
			    + scz[3] - (mad_118_).scz[3])
			   * (scz[0] - (mad_118_).scz[0]
			      + scz[1] - (mad_118_).scz[1]
			      + scz[2] - (mad_118_).scz[2]
			      + scz[3] - (mad_118_).scz[3]))
			  + ((scx[0] - (mad_118_).scx[0]
			      + scx[1] - (mad_118_).scx[1]
			      + scx[2] - (mad_118_).scx[2]
			      + scx[3] - (mad_118_).scx[3])
			     * (scx[0] - (mad_118_).scx[0]
				+ scx[1] - (mad_118_).scx[1]
				+ scx[2] - (mad_118_).scx[2]
				+ scx[3]
	...
	if (i_136_ != 0) {
		...
		fs[i_138_]
		    = (float) i + ((f - (float) i) * m.cos(i_136_)
				   - ((f_139_ - (float) i_135_)
				      * m.sin(i_136_)));
		fs_134_[i_138_]
		    = ((float) i_135_
		       + ((f - (float) i) * m.sin(i_136_)
			  + ((f_139_ - (float) i_135_)
			     * m.cos(i_136_))));
	    }
	}
