...
public class ContO
{
	...
	if ((m).loadnew) {
		...
		if (is_0_[i_27_] == 1) {
		    ...
		    if (i_28_ == 1) {
			if (Math.abs((p[i_27_]).oz[0]
				     - (p[i_27_]).oz[1])
			    > Math.abs((p[i_27_]).oy[0]
				       - ((p[i_27_]).oy
					  [1]))) {
			    if ((p[i_27_]).oz[0]
				> (p[i_27_]).oz[1]) {
				if ((p[i_27_]).oy[1]
				    ...
				    (p[i_27_]).fs = -1;
			    } else if ((p[i_27_]).oy[1]
				       > ((p[i_27_]).oy
					  [2]))
				(p[i_27_]).fs = -1;
			    else
				(p[i_27_]).fs = 1;
			} else if ((p[i_27_]).oy[0]
				   > (p[i_27_]).oy[1]) {
			    if ((p[i_27_]).oz[1]
				> (p[i_27_]).oz[2])
				(p[i_27_]).fs = -1;
			...
			if (Math.abs((p[i_27_]).oz[0]
				...
				if ((p[i_27_]).ox[1]
				    ...
				    (p[i_27_]).fs = 1;
			    } else if ((p[i_27_]).ox[1]
				       > ((p[i_27_]).ox
					  [2]))
				(p[i_27_]).fs = 1;
			    else
				(p[i_27_]).fs = -1;
			} else if ((p[i_27_]).ox[0]
				   > (p[i_27_]).ox[1]) {
			    if ((p[i_27_]).oz[1]
				> (p[i_27_]).oz[2])
				(p[i_27_]).fs = 1;
	...
	for (int i_109_ = 0; i_109_ < 4; i_109_++) {
	    ...
	    p[i_109_]
		= new Plane(m, t, is_112_,
			    is_114_, is_113_, 6, is_106_, 3, -8, 0, 0, 0, 0,
			    disline, 0, true, 0, false);
	}
	...
	for (int i_116_ = 0; i_116_ < 3; i_116_++) {
		...
		is_106_[i_116_]
		    = (int) ((float) ((m).cpol[i_116_]
				      + ((m).cgrnd
					 [i_116_]))
			     / (2.15F + f_100_));
	}
	p[4]
	    = new Plane(m, t, is_96_, is_97_,
	...
	(t).radz[(t).nt]
	    = is_118_[0] - is_118_[1];
	(t).x[(t).nt]
	    = (is_117_[0] + is_117_[1]) / 2 + x;
	(t).z[(t).nt]
	    = (is_118_[0] + is_118_[1]) / 2 + z;
	(t).zy[(t).nt] = 0;
	(t).xy[(t).nt] = 0;
	    ...
	    = ((m).cz
	       + (int) (((float) (y
			...
			+ ((float) (i_124_ - (m).cz)
			   * m
				 .cos((m).zy))));
	int i_126_ = (xs(i + maxR, i_125_)
		      - xs(i - maxR, i_125_));
	if ((xs(i + maxR * 2, i_125_)
	     > (m).iw)
	    && (xs(i - maxR * 2, i_125_)
		< (m).w)
	    && i_125_ > -maxR
				...
				= ((m).cz
				   + (int) (((float) ((
					    ...
					    + ((float) (i_124_
						  ...
						  ((m)
						   .zy)))));
			    if (ys(i_132_ + maxR, i_133_) > 0
				&& (ys(i_132_ - maxR, i_133_)
				    < (m).h)) {
				for (int i_134_ = 0;
				     i_134_ < npl; i_134_++)
				    p[i_134_].s
		...
		= ((m).cy
		   + (int) (((float) (y
			    ...
			    - ((float) (i_124_
			       ...
			       * m
				     .sin((m).zy))));
	    if ((ys(i_136_ + maxR, i_125_)
		 > (m).ih)
		&& (ys(i_136_ - maxR, i_125_)
		    < (m).h)) {
		if (elec
		    && (m).noelec == 0)
	...
	int i_148_ = 1;
	int i_149_;
	for (i_149_ = Math.abs(zy); i_149_ > 270;
	     i_149_ -= 360) {
	    /* empty */
	}
	...
	if (i_149_ > 90)
	    i_148_ = -1;
	is[0] = (int) ((double) keyx[0] * 1.2
		       + (double) x
		       - (double) (m).x);
	is_147_[0]
	    = (int) ((double) ((keyz[0] + 30) * i_148_) * 1.2
		     + (double) z
		     - (double) (m).z);
	is[1] = (int) ((double) keyx[1] * 1.2
		       + (double) x
		       - (double) (m).x);
	is_147_[1]
	    = (int) ((double) ((keyz[1] + 30) * i_148_) * 1.2
		     + (double) z
		     - (double) (m).z);
	is[2] = (int) ((double) keyx[3] * 1.2
		       + (double) x
		       - (double) (m).x);
	is_147_[2]
	    = (int) ((double) ((keyz[3] - 30) * i_148_) * 1.2
		     + (double) z
		     - (double) (m).z);
	is[3] = (int) ((double) keyx[2] * 1.2
		       + (double) x
		       - (double) (m).x);
	is_147_[3]
	    = (int) ((double) ((keyz[2] - 30) * i_148_) * 1.2
		     + (double) z
		     - (double) (m).z);
	rot(is, is_147_, x - (m).x,
	    z - (m).z,
	    xz, 4);
	int i_150_
	...
	if ((t).ncx != 0
	    ...
	    if (i_155_ < 0)
		i_155_ = 0;
	    for (int i_156_ = ((t).sect[i_154_]
			       [i_155_]).length - 1;
		 i_156_ >= 0; i_156_--) {
		int i_157_ = ((t).sect[i_154_]
			      [i_155_][i_156_]);
		int i_158_ = 0;
		for (int i_159_ = 0; i_159_ < 4; i_159_++) {
		    if ((Math.abs((t).zy[i_157_])
			 != 90)
			&& (Math.abs((t).xy[i_157_])
			    != 90)
			&& (t).rady[i_157_] != 801
			&& (Math.abs(is[i_159_]
				     - ((t).x[i_157_]
	...
	for (int i_165_ = 0; i_165_ < 4; i_165_++) {
	    ...
	    if (is_146_[i_165_] < (m).ih
		|| is_147_[i_165_] < 10)
		i_161_++;
	    if (is_146_[i_165_] > (m).h
		|| is_147_[i_165_] < 10)
		i_162_++;
	    if (is[i_165_] < (m).iw
		|| is_147_[i_165_] < 10)
	...
	if (bool) {
		...
		if (i > (m).fade[i_166_]) {
		    i_150_ = ((i_150_ * (m).fogd
			       + (m).cfade[0])
			      / ((m).fogd + 1));
		    i_151_ = ((i_151_ * (m).fogd
			       + (m).cfade[1])
			      / ((m).fogd + 1));
		    i_152_ = ((i_152_ * (m).fogd
			       + (m).cfade[2])
			      / ((m).fogd + 1));
		}
	    }
	...
	if ((fcnt == 1 || fcnt > 2)
		...
		z - (m).y,
		zy, 4);
	    rot(is, is_171_, x - (m).x,
		z - (m).z,
		xz, 4);
	    rot(is, is_171_, (m).cx,
		(m).cz, (m).xz,
		4);
	    rot(is_170_, is_171_, (m).cy,
		(m).cz, (m).zy,
		4);
	    int i = 0;
	    int i_172_ = 0;
	...
	for (int i = 0; i < 4; i++) {
	    ...
	    int i_182_
		= (int) ((float) edl[i]
			 + (190.0F - m.random() * 380.0F));
	    int i_183_
		= (int) ((float) edr[i]
			 + (190.0F - m.random() * 380.0F));
	    int i_184_ = (int) (m.random() * 126.0F);
	    int i_185_ = (int) (m.random() * 126.0F);
	    int[] is = new int[8];
		...
		if (is_194_[i_195_] < (m).ih
		    || is_187_[i_195_] < 10)
		    i_189_++;
		if (is_194_[i_195_] > (m).h
		    || is_187_[i_195_] < 10)
		    i_190_++;
		if (is_193_[i_195_] < (m).iw
		    || is_187_[i_195_] < 10)
		    i_191_++;
		if (is_193_[i_195_] > (m).w
		    || is_187_[i_195_] < 10)
		    i_192_++;
	    }
		...
		if (i_198_ < 0)
		    i_198_ = 0;
		i_196_ = ((i_196_ * 2 + 214 * (elc[i] - 1))
			  / (elc[i] + 1));
		i_197_ = ((i_197_ * 2 + 236 * (elc[i] - 1))
			  / (elc[i] + 1));
		if ((m).trk == 1) {
		    i_196_ = 255;
		    i_197_ = 128;
	...
	if ((double) f_206_ > 0.2 && !bool_205_) {
		...
		ust = 0;
	    if (!bool) {
		float f_207_ = m.random();
		sx[ust]
		    = (int) ((f + (float) x * f_207_)
			     / (1.0F + f_207_));
		sz[ust]
		    = (int) ((f_200_ + (float) z * f_207_)
			     / (1.0F + f_207_));
		sy[ust]
		    = (int) ((f_199_ + (float) y * f_207_)
			     / (1.0F + f_207_));
	    } else {
		sx[ust]
		    = (int) ((f + (float) (x + i_201_)) / 2.0F);
		sz[ust]
		    = (int) ((f_200_ + (float) (z + i_202_))
			     / 2.0F);
		sy[ust] = (int) f_199_;
	    }
	    if (sy[i] > 250)
	...
	if (bool && sav[i] > dist
		...
		if (i_211_ < 0)
		    i_211_ = 0;
		for (int i_212_ = 0;
		     i_212_ < ((t).sect[i_210_]
			       [i_211_]).length;
		     i_212_++) {
		    int i_213_ = ((t).sect[i_210_]
				  [i_211_][i_212_]);
		    if ((Math.abs((t).zy[i_213_])
			 != 90)
			&& (Math.abs((t).xy[i_213_])
			    != 90)
			&& (Math.abs(sx[i]
				     - (t).x[i_213_])
			    < (t).radx[i_213_])
	    ...
	    is[0] = xs((int) ((float) i_220_
			      + smag[i][0] * 0.9238F * 1.5F),
		       i_221_);
	    is_223_[0]
		= ys((int) ((float) i_222_
			    + smag[i][0] * 0.3826F * 1.5F),
		     i_221_);
	    is[1] = xs((int) ((float) i_220_
			      + smag[i][1] * 0.9238F * 1.5F),
		       i_221_);
	    is_223_[1]
		= ys((int) ((float) i_222_
			    - smag[i][1] * 0.3826F * 1.5F),
		     i_221_);
	    ...
	    is[4] = xs((int) ((float) i_220_
			      - smag[i][4] * 0.9238F * 1.5F),
		       i_221_);
	    is_223_[4]
		= ys((int) ((float) i_222_
			    - smag[i][4] * 0.3826F * 1.5F),
		     i_221_);
	    is[5] = xs((int) ((float) i_220_
			      - smag[i][5] * 0.9238F * 1.5F),
		       i_221_);
	    is_223_[5]
		= ys((int) ((float) i_222_
			    + smag[i][5] * 0.3826F * 1.5F),
		     i_221_);
	    is[6] = xs((int) ((float) i_220_
			      - smag[i][6] * 0.3826F * 1.7F),
		       i_221_);
	    is_223_[6] = ys((int) ((float) i_222_
				   + smag[i][6] * 0.9238F),
			    i_221_);
	    is[7] = xs((int) ((float) i_220_
			      + smag[i][7] * 0.3826F * 1.7F),
		       i_221_);
		...
		if (is_223_[i_230_] < (m).ih
		    || i_221_ < 10)
		    i_226_++;
		if (is_223_[i_230_] > (m).h
		    || i_221_ < 10)
		    i_227_++;
		...
		for (int i_234_ = 0; i_234_ < 16; i_234_++) {
		    if (sav[i]
			> (m).fade[i_234_]) {
			i_231_ = ((i_231_ * (m).fogd
				   + (m).cfade[0])
				  / ((m).fogd + 1));
			i_232_ = ((i_232_ * (m).fogd
				   + (m).cfade[1])
				  / ((m).fogd + 1));
			i_233_ = ((i_233_ * (m).fogd
				   + (m).cfade[2])
				  / ((m).fogd + 1));
		    }
		}
	...
	if (i != 1) {
		...
		= (int) (f - ((float) sprkat
			      * m.sin(xz)));
	    sry
		= (int) (f_235_ - ((float) sprkat
				   * m.cos(zy)
				   * m.cos(xy)));
	    srz
		= (int) (f_236_ + ((float) sprkat
				   * m.cos(xz)));
	    sprk = 1;
	} else {
	    sprk++;
	    if (sprk == 4) {
		srx = (int) ((float) x + f_237_);
		sry = (int) f_235_;
		srz = (int) ((float) z + f_239_);
		sprk = 5;
	    } else {
		srx = (int) f;
		sry = (int) f_235_;
	...
	if (bool && sprk != 0) {
	    int i = (int) (Math.sqrt((double) ((rcx
						* rcx)
					       + (rcy
						  * rcy)
					       + (rcz
